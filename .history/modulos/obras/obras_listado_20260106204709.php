...
// --- CONSULTA MAESTRA ---
// Se mantiene la exclusión de 'ANTICIPO' y 'ANULADOS' para el avance físico
$sql = "
  SELECT 
    ...
    -- DATOS REALES
    -- Total Pagado (Incluye todo lo aprobado, incluso anticipos)
    (SELECT COALESCE(SUM(monto_neto_pagar),0) FROM certificados WHERE obra_id=o.id AND estado='APROBADO') as total_certificado,
    
    -- Avance Físico Real (Excluyendo ANTICIPO y ANULADOS)
    (SELECT COALESCE(SUM(avance_fisico_mensual),0) 
     FROM certificados 
     WHERE obra_id=o.id AND estado != 'ANULADO' AND tipo != 'ANTICIPO') as avance_real_pct,
    
    -- Tooltip: Desglose (Excluyendo ANTICIPO)
    (SELECT GROUP_CONCAT(CONCAT('#', nro_certificado, ' (', tipo, '): ', ROUND(avance_fisico_mensual, 2), '%') SEPARATOR '\n') 
     FROM certificados 
     WHERE obra_id=o.id AND estado != 'ANULADO' AND tipo != 'ANTICIPO') as tooltip_avance,
     
    -- Último Periodo (Cualquiera activo)
                ...
                <table id="tablaObras" class="table table-hover align-middle mb-0 table-vcenter" style="font-size: 0.95rem; width:100%">
                    <thead class="table-light text-secondary small text-uppercase">
                        <tr>
                            ...
                            <th class="text-center" style="width: 180px;">Acciones</th> </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex justify-content-between small mb-1 fw-bold">
                                    <span class="text-success cursor-pointer" data-bs-toggle="tooltip" data-bs-html="true" title="<b>Desglose Físico:</b><br><?= $tooltipDetalle ?>" style="cursor:help; text-decoration: underline dotted;">
                            ...
                            <td class="text-center">
                                <div class="btn-group" role="group">
                                    ...
                                    <a href="obras_evento.php?obra_id=<?= $o['id'] ?>" class="btn btn-info btn-action shadow-sm text-white" title="Eventos / Vedas" data-bs-toggle="tooltip"><i class="bi bi-calendar-range fs-6"></i></a>

                                    <a href="obras_form.php?id=<?= $o['id'] ?>" class="btn btn-dark btn-action shadow-sm" title="Editar Obra" data-bs-toggle="tooltip"><i class="bi bi-pencil-fill fs-6"></i></a>
                                </div>
                            </td>
                        </tr>
                        <?php endforeach; ?>
