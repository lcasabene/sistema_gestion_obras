...
// --- CONSULTA MAESTRA ---
// Se agrega la columna 'tooltip_avance' con el detalle de la suma
$sql = "
  SELECT 
    ...
    (SELECT COALESCE(SUM(avance_fisico_mensual),0) FROM certificados WHERE obra_id=o.id AND estado != 'ANULADO') as avance_real_pct,
    
    -- >>> NUEVO: DETALLE PARA TOOLTIP (Lista qué certificados componen la suma) <<<
    (SELECT GROUP_CONCAT(CONCAT('Cert #', nro_certificado, ': ', ROUND(avance_fisico_mensual, 2), '%') SEPARATOR '\n') 
     FROM certificados WHERE obra_id=o.id AND estado != 'ANULADO') as tooltip_avance,
...
<style>
    ...
    /* Estilo para que el tooltip respete los saltos de linea */
    .tooltip-inner { white-space: pre-wrap; text-align: left; max-width: 300px; }
</style>
                ...
                <table id="tablaObras" class="table table-hover align-middle mb-0 table-vcenter" style="font-size: 0.95rem; width:100%">
                    ...
                    <tbody>
                        <?php foreach($obras as $o): 
                            ...
                            if(empty($tooltipDetalle)) $tooltipDetalle = "Sin certificados activos";
                        ?>
                        <tr>
                            <td>
                            ...
                            <td>
                                <div class="d-flex justify-content-between small mb-1 fw-bold">
                                    <span class="text-success cursor-pointer" data-bs-toggle="tooltip" data-bs-html="true" title="<b>Desglose:</b><br><?= $tooltipDetalle ?>" style="cursor:help; text-decoration: underline dotted;">
...
$(document).ready(function() {
    ...
    // Inicializar Tooltips con soporte HTML
