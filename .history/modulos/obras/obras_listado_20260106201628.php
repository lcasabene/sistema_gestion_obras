...
// --- CONSULTA MAESTRA ---
// Se agregan: o.monto_original, o.fecha_fin (Plazo) y subconsulta para ultimo_periodo
$sql = "
  SELECT 
    o.id, o.denominacion, o.monto_actualizado, o.monto_original, o.expediente, o.fecha_fin,
    e.nombre AS estado,
...
try {
    ...
    // Si falla por 'fecha_fin', avisa al usuario
    $errorMsg = $e->getMessage();
    if(strpos($errorMsg, 'Unknown column') !== false && strpos($errorMsg, 'fecha_fin') !== false) {
        $errorMsg = "La columna 'fecha_fin' no existe en la tabla obras. Por favor verifica el nombre del campo de plazo/fecha en tu base de datos.";
    }
    echo "<div class='container my-4'><div class='alert alert-danger'><strong>Error Base de Datos:</strong> " . $errorMsg . "</div></div>";
    $obras = [];
}
            ...
            <div class="table-responsive p-3">
                <table id="tablaObras" class="table table-hover align-middle mb-0 table-vcenter" style="font-size: 0.95rem; width:100%">
                    <thead class="table-light text-secondary small text-uppercase">
                        <tr>
                            <th>Obra / Plazo</th> <th class="text-center">Estado</th>
                            <th class="text-center" style="width: 20%;">Avance Físico</th> <th class="text-end">Finanzas (Orig. / Act.)</th> <th class="text-center" style="width: 160px;">Acciones</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach($obras as $o): 
                            ...
                            // Formato de Fecha Fin (Si es null muestra -)
                            $fechaFin = $o['fecha_fin'] ? date('d/m/Y', strtotime($o['fecha_fin'])) : '-';
...
$(document).ready(function() {
    ...
    $('#tablaObras').DataTable({
        ...
        order: [[0, 'desc']], 
        columnDefs: [
            { orderable: false, targets: 4 } // Ajustado el índice de la columna no ordenable
        ],
        pageLength: 10,
