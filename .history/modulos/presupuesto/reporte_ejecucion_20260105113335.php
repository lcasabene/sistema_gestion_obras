...
// ---------------------------------------------------------
// Ajustamos las rutas según tu indicación exacta
require_once __DIR__ . '/../../config/database.php';
require_once __DIR__ . '/../../auth/middleware.php';

// Intentamos capturar la conexión. 
// Si tu archivo database.php usa $pdo o $db en vez de $conn, aquí lo igualamos.
global $conn;
if (!isset($conn) && isset($pdo)) { $conn = $pdo; }
if (!isset($conn) && isset($db)) { $conn = $db; }

if (!$conn) {
    die("Error Crítico: No se pudo detectar la variable de conexión a la base de datos (\$conn, \$pdo o \$db). Revisa config/database.php.");
}
...
// B. Consulta SQL Adaptada a tus Imágenes
$sql = "SELECT 
            o.id, 
            o.denominacion, 
            pe.monto_def, 
            pe.fufi,
            ci.periodo as fecha, 
            -- Sumamos base + redeterminación para el monto total proyectado
            (COALESCE(ci.monto_base, 0) + COALESCE(ci.redeterminacion, 0)) as monto_proyectado
        FROM obras o
        
        -- 1. Presupuesto (Datos financieros generales)
        LEFT JOIN presupuesto_ejecucion pe 
            ON o.id = pe.id_obra
            
        -- 2. Curva Cabecera (Tabla 'curvas', filtramos la vigente)
        LEFT JOIN curvas c 
            ON o.id = c.obra_id AND c.es_vigente = 1
            
        -- 3. Items de la Curva (Tabla 'curva_items', detalle mensual)
        -- OJO: Según tu imagen, la FK es 'version_id' y la fecha es 'periodo'
        LEFT JOIN curva_items ci 
            ON c.id = ci.version_id AND ci.periodo >= :fecha_corte
...
try {
    ...
    echo "<div class='alert alert-danger m-3'>
            <strong>Error SQL:</strong> " . $e->getMessage() . "
          </div>";
    $resultados = [];
}

// C. Procesamiento (Pivote: Filas -> Columnas)
$obras = [];
$todos_los_meses = [];
...
foreach ($resultados as $fila) {
    ...
    // Crear estructura de la obra si es nueva en el loop
    if (!isset($obras[$id])) {
        $obras[$id] = [
    ...
    // Si hay fecha, procesamos el monto
    if (!empty($fila['fecha'])) {
        $mes_clave = date('Y-m', strtotime($fila['fecha'])); // Clave ej: 2026-02
        
        if (!isset($obras[$id]['proyecciones'][$mes_clave])) {
            $obras[$id]['proyecciones'][$mes_clave] = 0;
        }
        
        // Acumulamos el monto calculado (base + redeterminación)
        $obras[$id]['proyecciones'][$mes_clave] += $fila['monto_proyectado'];
        
        // Guardar mes para encabezados
        if (!in_array($mes_clave, $todos_los_meses)) {
            $todos_los_meses[] = $mes_clave;
        }
    }
}
// Ordenar meses cronológicamente para las columnas
                ...
                <table id="tablaReporte" class="table table-striped table-bordered table-hover w-100 table-sm">
                    <thead class="table-dark">
                        <tr>
                            ...
                            ?>
                                <th class="align-middle text-center bg-secondary" style="min-width: 90px;"><?php echo $label; ?></th>
                            <?php endforeach; ?>
                        </tr>
                    </thead>
                    <tbody>
...
$(document).ready(function() {
    ...
    $('#tablaReporte').DataTable({
        dom: 'Bfrtip',
        paging: false,  // Mostrar todo
        scrollX: true,  // Permitir scroll si hay muchos meses
        order: [[0, 'asc']], // Ordenar alfabéticamente por obra
        buttons: [
            {
                extend: 'excelHtml5',
                ...
                title: tituloReporte,
                footer: true // Incluye la fila de totales
            },
            {
                extend: 'print',
