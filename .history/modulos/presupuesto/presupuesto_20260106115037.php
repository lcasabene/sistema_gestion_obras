...
// Incluimos header desde la ruta correcta (../../)
    ...
    <div class="card shadow-sm mb-4 border-0">
            ...
            <form id="formFiltros" method="GET" class="row g-2 align-items-end">
                ...
                <div class="col-md-7 text-end">
                    <div class="btn-group shadow-sm">
                        ...
                        <a href="../../public/menu.php" class="btn btn-primary btn-sm">
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table id="tablaPresupuesto" class="table table-hover table-sm mb-0 align-middle" style="width:100%">
                    ...
                    <tbody>
                        ...
                        if (!empty($registros)):
                            foreach ($registros as $r): 
                                // Casting a float para evitar errores matemáticos
                                $cred_total = (float)($r['monto_def'] ?? 0) + (float)($r['monto_reep'] ?? 0);
                                $ejec = (float)($r['monto_ejec'] ?? 0);
                                $disponible = (float)($r['monto_disp'] ?? 0);
                            ...
                            <tr class="<?= $es_critico ? 'fila-alerta' : '' ?>">
                                <td class="text-center">
                                    <span class="badge bg-secondary badge-presupuesto">
                                        <?= htmlspecialchars($r['inci'] ?? '') ?>-<?= htmlspecialchars($r['ppal'] ?? '') ?>-<?= htmlspecialchars($r['ppar'] ?? '') ?>-<?= htmlspecialchars($r['spar'] ?? '') ?>
                                    </span>
                                </td>
                                <td class="text-center fw-bold text-primary"><?= htmlspecialchars($r['fufi'] ?? '') ?></td>
                                <td class="col-denominacion">
                                    <div class="fw-bold small"><?= htmlspecialchars($r['denominacion1'] ?? '') ?></div>
                                    <div class="deno-3 small"><?= htmlspecialchars($r['denominacion3'] ?? '') ?></div>
                                </td>
                                ...
                                <td><?= htmlspecialchars($r['cpn1'] ?? '') ?></td>
                                <td><?= htmlspecialchars($r['cpn2'] ?? '') ?></td>
                                <td><?= htmlspecialchars($r['cpn3'] ?? '') ?></td>
                                
                                <td class="text-center no-export">
                                    <button type="button" class="btn btn-outline-info btn-sm btn-detalle" 
                                            data-datos='<?= htmlspecialchars(json_encode($r), ENT_QUOTES, 'UTF-8') ?>'>
  ...
  <div class="modal-dialog modal-xl modal-dialog-scrollable">
    <div class="modal-content border-0 shadow">
      <div class="modal-header bg-dark text-white">
        <h5 class="modal-title">Ficha Técnica Presupuestaria</h5>
...
$(document).ready(function() {
    ...
    var table = $('#tablaPresupuesto').DataTable({
        ...
        columnDefs: [
            {
                targets: [7, 8, 9], // Columnas CPN Ocultas
                visible: false,     
                searchable: true    
            }
        ]
    ...
    // Lógica Filtro Crítico (>= 80%)
    $('#btnFiltroCritico').on('click', function() {
        if ($(this).hasClass('active')) {
            // Desactivar filtro
            $(this).removeClass('active').removeClass('btn-danger').addClass('btn-outline-danger').html('<i class="bi bi-filter"></i> Ver Críticos (≥ 80%)');
            table.column(6).search('').draw(); 
        } else {
            // Activar filtro
            $(this).addClass('active').removeClass('btn-outline-danger').addClass('btn-danger').html('<i class="bi bi-x-circle"></i> Ver Todos');
            // Buscamos la palabra "CRITICO" que insertamos en la columna 6 con PHP
    ...
    $('.btn-detalle').on('click', function() {
        ...
        for (const [key, value] of Object.entries(datos)) {
            // Omitimos campos técnicos internos
            ...
            <div class="col-md-3">
                <div class="card h-100 border-0 shadow-sm">
                    <div class="card-body p-2">
                        <label class="text-muted fw-bold small text-uppercase" style="font-size:0.6rem;">${key.replace(/_/g,' ')}</label>
                        <div class="small text-dark text-break fw-bold">${value || '-'}</div>
                    </div>
                </div>
            </div>`;
        }
