...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($id == 0) {
            $sql = "INSERT INTO certificados (obra_id, empresa_id, nro_certificado, tipo, periodo, fecha_medicion, fecha_firma, fecha_requisitos_cumplidos, fecha_vencimiento, monto_basico, monto_redeterminado, fri, monto_neto, monto_bruto) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            ...
            $sql = "UPDATE certificados SET obra_id=?, empresa_id=?, nro_certificado=?, tipo=?, periodo=?, fecha_medicion=?, fecha_firma=?, fecha_requisitos_cumplidos=?, fecha_vencimiento=?, monto_basico=?, monto_redeterminado=?, fri=?, monto_neto=?, monto_bruto=? WHERE id=?";
            $stmt = $pdo->prepare($sql);
