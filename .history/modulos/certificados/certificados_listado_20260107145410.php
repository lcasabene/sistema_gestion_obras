...
$sql = "SELECT c.*, o.denominacion as obra, e.razon_social as empresa 
        ...
        WHERE o.activo = 1 
        ORDER BY c.id DESC"; // Ordenamos por ID DESC para ver los nuevos primero
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <span class="badge bg-info bg-opacity-10 text-dark border border-info">
                                    <?= number_format($c['avance_fisico_mensual'], 2) ?>%
                                </span>
                            </td>
                            <td class="text-end fw-bold text-success">
    <small class="text-muted" style="font-size:10px;">Raw: <?= $c['monto_neto_pagar'] ?></small><br>
...
$(document).ready(function() {
    $('#tablaCertificados').DataTable({
        ...
        pageLength: 25,
        columnDefs: [
            { orderable: false, targets: 7 } // No ordenar columna acciones
        ]
    });
});
</script>
