...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Mapeo Crítico: ¿Dónde va la plata?
        $monto_basico = 0; 
        $monto_redet = 0;

        if ($tipo == 'ORDINARIO' || $tipo == 'ANTICIPO') {
            $monto_basico = $monto_bruto; // El anticipo SE GUARDA en monto_basico
        ...
        if (isset($_POST['fuente_id'])) {
            ...
            for ($i = 0; $i < count($_POST['fuente_id']); $i++) {
                if ($_POST['fuente_id'][$i]) {
                    // Importante: Guardamos el monto tal cual viene del form (ya limpio por $f)
