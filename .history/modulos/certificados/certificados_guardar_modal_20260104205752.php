...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // CORRECCIÓN: Limpieza agresiva del Tipo para evitar fallos de comparación
        $tipoRaw = $_POST['tipo'] ?? '';
        ...
        // Mapeo Crítico: Aseguramos que el Básico reciba el valor
        $monto_basico = 0; 
        $monto_redet = 0;

        // Usamos strpos para ser más flexibles (ej: si llega "ANTICIPO FINANCIERO" funciona igual)
        if (strpos($tipo, 'ORDINARIO') !== false || strpos($tipo, 'ANTICIPO') !== false) {
            $monto_basico = $monto_bruto; 
        } elseif (strpos($tipo, 'REDETERMINACION') !== false) {
