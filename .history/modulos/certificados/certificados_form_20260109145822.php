...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $monto_neto_pagar = $monto_bruto; // Usamos el nombre correcto para la variable también
        ...
        // --- C. INSERT O UPDATE EN BASE DE DATOS (CORREGIDO: monto_neto_pagar) ---
        if ($id == 0) {
            // Se cambió monto_neto por monto_neto_pagar
            ...
            $stmt->execute([$obra_id, $empresa_id, $nro_cert, $tipo, $periodo, $f_medicion, $f_firma, $f_requisitos, $f_vencimiento, $monto_basico, $monto_redet, $fri, $monto_neto_pagar, $monto_bruto]);
            $id = $pdo->lastInsertId();
        } else {
            // Se cambió monto_neto por monto_neto_pagar
            ...
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$obra_id, $empresa_id, $nro_cert, $tipo, $periodo, $f_medicion, $f_firma, $f_requisitos, $f_vencimiento, $monto_basico, $monto_redet, $fri, $monto_neto_pagar, $monto_bruto, $id]);
        }

        // --- D. VINCULACIÓN: ÓRDENES DE PAGO ---
...
function setEmpresa(sel) {
    ...
    // Opcional: Limpiar listado de facturas al cambiar de empresa
