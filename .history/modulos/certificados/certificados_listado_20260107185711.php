...
function fmtM($v) {
    // Formato monetario seguro: soporta números y strings con separadores es-AR.
    if ($v === null || $v === '') return number_format(0, 2, ',', '.');
    if (is_float($v) || is_int($v)) return number_format((float)$v, 2, ',', '.');

    $s = trim((string)$v);

    // Si viene como "1.600.000.000,00" o "1600000000,00"
    if (strpos($s, ',') !== false) {
                ...
                <table id="tablaCertificados" class="table table-hover align-middle mb-0" style="width:100%">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge bg-light text-dark border"><?= htmlspecialchars($c['periodo']) ?></span>
                            </td>
                            <td>
                                <span class="fw-bold">Nº <?= htmlspecialchars($c['nro_certificado']) ?></span>
                                <div class="small text-muted" style="font-size: 0.75rem;"><?= htmlspecialchars($c['tipo']) ?></div>
                            </td>
                            <td>
                            ...
                            <td class="text-center">
                                ...
                                ?>
                                <span class="badge bg-<?= $cls ?>"><?= htmlspecialchars($c['estado']) ?></span>
                            </td>
                            <td class="text-center">
                                <div class="btn-group shadow-sm">
                                    <a href="certificados_form.php?id=<?= (int)$c['id'] ?>" class="btn btn-sm btn-outline-primary" title="Editar">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    
                                    <a href="certificados_eliminar.php?id=<?= (int)$c['id'] ?>" 
