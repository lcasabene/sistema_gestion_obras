...
// --------------------------------------------------------------------------
// Se traen ordenados primero Anticipo (si existiera lógica para ello) y luego por fecha
...
// Extraemos los IDs de los items para hacer una búsqueda eficiente de certificados
$idsItems = array_column($itemsCurva, 'id');
$idsString = !empty($idsItems) ? implode(',', array_map('intval', $idsItems)) : '0';

// --------------------------------------------------------------------------
// 4. CERTIFICADOS REALES (VINCULACIÓN ESTRICTA)
// --------------------------------------------------------------------------
// Solo traemos certificados que tengan curva_item_id coincidente con esta versión.
$mapaCertificados = []; 
$ultimoPeriodoReal = null; // Para cortar el gráfico
...
if (!empty($idsItems)) {
    ...
    while ($row = $stmtC->fetch(PDO::FETCH_ASSOC)) {
        // Estructura: [ID_ITEM] => [TIPO] => [Lista de Certificados]
        $mapaCertificados[$row['curva_item_id']][$row['tipo']][] = $row;
        
        // Guardamos la última fecha real registrada para el gráfico
...
foreach ($itemsCurva as $i) {
    // Excluir anticipo financiero del gráfico de curva S habitual
    ...
    // Acumulado Real
    // Sumamos todo lo "ORDINARIO" que esté vinculado a este item específico
    $avanceItem = 0;
    ...
    // Lógica de corte del gráfico:
    // Si hay avance en este mes O ya veníamos acumulando avance O la fecha es menor a la última real conocida
    if ($avanceItem > 0 || $acumReal > 0 || ($ultimoPeriodoReal && $per <= $ultimoPeriodoReal)) {
        $acumReal += $avanceItem;
        $dataReal[] = $acumReal;
    } else {
        $dataReal[] = null; // Cortar línea
    }
}
?>
...
<div class="container-fluid px-4 my-4">
    ...
    <div class="collapse show mb-4" id="collapseGraph">
        <div class="card shadow-sm border-0"><div class="card-body" style="height: 350px;"><canvas id="chartCurva"></canvas></div></div>
    </div>
            ...
            <table class="table table-bordered table-hover mb-0 table-vcenter text-center">
                ...
                <tbody>
                    <?php foreach($itemsCurva as $item): 
                        ...
                        // RECUPERAR DATOS REALES (VINCULACIÓN ESTRICTA)
                        // Buscamos en el array $mapaCertificados usando $itemId como KEY
                        ...
                        // Si el item planificado es anticipo, usaremos esa colección
...
<div class="modal fade" id="modalCertificadoUpd" tabindex="-1">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="../certificados/certificados_guardar_modal.php" method="POST" id="formCertificadoUpd" target="_blank">
                <input type="hidden" name="obra_id" value="<?= $cabecera['obra_real_id'] ?>">
                <input type="hidden" name="version_prev_id" value="<?= $versionId ?>"> <input type="hidden" name="periodo" id="chk_modalPeriodo">
                ...
                <div class="modal-footer bg-light p-2">
                    ...
                    <button type="button" class="btn btn-warning btn-sm px-4 fw-bold text-dark" onclick="verificarDatos()">
    🔍 VERIFICAR ID
</button>
                </div>
            </form>
...
function abrirModalEdit(id, itemId) {
    // IMPORTANTE: Asegurate de tener este endpoint en backend
...
function resetModal(tipo, periodo, id, itemId) {
    ...
    document.getElementById('chk_modalCurvaItemId').value = itemId; // EL DATO IMPORTANTE
...
function toggleCampos(tipo) {
    ...
    if(tipo==='ORDINARIO' || tipo==='ANTICIPO') {
        ...
        if(tipo === 'ANTICIPO') document.getElementById('chk_inputAnticipoPct').value = 0; // Anticipo no descuenta anticipo
    } 
    if(tipo==='REDETERMINACION') {
        divRed.forEach(e => e.classList.remove('d-none'));
    }
}

// Lógica de cálculo (Front)
function calcBasico() {
    let pct = parseFloat(document.getElementById('chk_inputAvance').value) || 0;
    // Usamos el monto de la obra actual para calcular el monto del certificado
...
function calcDeducciones() {
    ...
    // Reverse calc (Si edito monto, ajusto el %)
...
// Mascaras simples al perder foco
function bindMasks() { 
...
function verificarDatos() {
    // 1. Obtener el valor oculto
    var idVinculo = document.getElementById('chk_modalCurvaItemId').value;
    var nroCert = document.getElementById('chk_inputNroCert').value;
    var monto = document.getElementById('chk_inputMontoBruto').value;

    // 2. Mostrar alerta
    if (idVinculo === "" || idVinculo === "0" || idVinculo === null) {
        alert("❌ ERROR: El ID de vinculación está VACÍO.\n\nEl problema está en el Javascript 'abrirModalNew' o 'abrirModalEdit'.");
    } else {
        alert("✅ CORRECTO: El ID de vinculación es: " + idVinculo + "\n\nDatos listos para enviar:\nCert: " + nroCert + "\nMonto: " + monto + "\n\nSi ves esto, el formulario funciona bien. El problema es el archivo PHP de guardado.");
        
        // Opcional: Si quieres enviar de verdad después de ver el alerta, descomenta la linea de abajo:
        // document.getElementById('formCertificadoUpd').submit(); 
    }
}
</script>
</body>
