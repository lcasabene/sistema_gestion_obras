...
<head>
    ...
    <style>
        ...
        .nro-cert { font-size: 0.9em; font-weight: 700; color: #0d6efd; background: #e7f1ff; padding: 2px 6px; border-radius: 4px; border: 1px solid #cce5ff; white-space: nowrap; display: inline-block; margin-right: 4px;}
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            ...
            <div class="text-muted small">
                ...
                Monto: $ <?= fmtM($cabecera['monto_actualizado']) ?>
            </div>
        </div>
        <div>
            <a href="curva_export.php?version_id=<?= $versionId ?>" class="btn btn-success btn-sm shadow-sm me-1"><i class="bi bi-file-excel"></i> Excel</a>
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-hover mb-0 table-vcenter text-center">
                <thead class="table-dark text-uppercase small">
                    <tr>
                        <th rowspan="2" style="width: 80px;">Periodo</th>
                        <th colspan="2" class="bg-secondary border-end">Planificación</th>
                        <th colspan="4" class="bg-success border-end">Certificado Básico / Anticipo</th>
                    ...
                    <tr>
                        ...
                        <th class="bg-success text-white small" style="width: 70px;">ID</th>
                    ...
                    <tr>
                        ...
                        <td class="col-basico">
                            <?php if($esAnticipo && !empty($certsAnticipo)): ?>
                                <?php foreach($certsAnticipo as $ca): ?>
                                    <span class="nro-cert">CO Nº <?= $ca['nro_certificado'] ?></span>
                                <?php endforeach; ?>
                            <?php elseif(!empty($certsBasicos)): ?>
                                <?php foreach($certsBasicos as $cb): ?>
                                    <div class="mb-1"><span class="nro-cert">CO Nº <?= $cb['nro_certificado'] ?></span></div>
                                <?php endforeach; ?>
                            <?php else: ?> - <?php endif; ?>
                        </td>
            ...
            <form action="../certificados/certificados_guardar_modal.php" method="POST" id="formModal">
                ...
                <div class="modal-body">
                    <div class="row align-items-center mb-3">
                        <div class="col-md-6">
                            <div class="alert alert-secondary py-1 small mb-0 fw-bold">
                                <i class="bi bi-info-circle"></i> Obra: $ <?= fmtM($cabecera['monto_actualizado']) ?>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="input-group input-group-sm">
                                <span class="input-group-text bg-dark text-white fw-bold">N° Cert.</span>
                                <input type="number" name="nro_certificado" id="inputNroCert" class="form-control fw-bold" placeholder="Auto">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row mb-3 bg-light p-2 rounded mx-0 border">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="monto_bruto" id="inputMontoBruto" class="form-control fw-bold text-end monto" required onkeyup="formatLive(this)" oninput="calcDeducciones()">
                        </div>
                    </div>
                    ...
                    <div class="row mb-3 mx-0">
                        ...
                        <div class="col-md-4 mb-2">
                            ...
                            <div class="input-group input-group-sm mb-1"><span class="input-group-text">5%</span><div class="input-group-text bg-white"><input class="form-check-input mt-0" type="checkbox" name="fondo_reparo_sustituido" id="checkSustituido" value="1" onchange="calcDeducciones()"><label class="small ms-1 mb-0" for="checkSustituido">Sust.</label></div></div>
                    ...
                    <div class="row mb-3 mx-0 border-top pt-2">
                        ...
                        <div class="col-12" id="containerFacturas"></div>
...
<div class="modal fade" id="modalArca" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered"><div class="modal-content"><div class="modal-header bg-info text-white py-2"><h6 class="modal-title">Facturas AFIP</h6><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body p-2" id="bodyArca"></div></div></div>
</div>
...
new Chart(ctx, { type: 'line', data: { labels: <?= json_encode($labels) ?>, datasets: [ { label: 'Plan', data: <?= json_encode($dataPlan) ?>, borderColor: '#0d6efd', backgroundColor: 'rgba(13,110,253,0.1)', fill: true }, { label: 'Real', data: <?= json_encode($dataReal) ?>, borderColor: '#198754' } ] }, options: { maintainAspectRatio: false } });
...
function abrirModalEdit(id) {
    ...
    .then(r => r.json())
    .then(data => {
        if(data.error) { alert("Error al cargar datos: " + data.error); return; }
        ...
        if(data.tipo==='ORDINARIO' || data.tipo==='ANTICIPO') {
            ...
            document.getElementById('inputAvance').value = data.avance_fisico_mensual;
        }
        ...
        if(data.facturas) data.facturas.forEach(f => agregarFacturaUI(f.comprobante_arca_id, f.numero, f.importe_total));
        
        new bootstrap.Modal(document.getElementById('modalCert')).show();
    })
    .catch(err => alert("Error de conexión: Asegúrese de tener el archivo api_get_certificado.php en la carpeta certificados."));
}
...
function resetModal(tipo, periodo, id) {
    ...
    document.getElementById('containerFacturas').innerHTML = '<div class=\"text-center text-muted small py-2\" id=\"msgNoFacturas\">Sin facturas</div>';
...
function calcDeducciones() {
    ...
    // Reverse calc del porcentaje si edito monto a mano (solo ordinario/anticipo)
    if(document.activeElement.id === 'inputMontoBruto' && montoObra > 0 && (tipo==='ORDINARIO' || tipo==='ANTICIPO')) {
    ...
    // 2. Dev. Anticipo (FIXED: Si es ANTICIPO, no descontar)
    let ant = 0;
    if(tipo !== 'ANTICIPO') {
    ...
    recalcularFuentes(); // Forzar recálculo
    validarFacturas();
}
...
function renderFuentes(lista) {
    ...
    div.innerHTML = '';
    lista.forEach(f => {
        let montoShow = (typeof f.monto_asignado === 'number') ? fmtM(f.monto_asignado) : f.monto_asignado; 
        if(!montoShow) montoShow = "0,00";
        div.innerHTML += `<div class="row mb-1 g-1 align-items-center small"><div class="col-7">${f.nombre} (${f.porcentaje}%)<input type="hidden" name="fuente_id[]" value="${f.fuente_id}"><input type="hidden" name="fuente_pct[]" value="${f.porcentaje}"></div><div class="col-5"><input type="text" name="fuente_monto[]" class="input-fufi monto form-control form-control-sm" value="${montoShow}" onkeyup="formatLive(this)"></div></div>`;
    });
}
...
function abrirModalArca() {
    ...
    fetch('../../modulos/certificados/api_get_facturas.php?cuit='+cuit).then(r=>r.text()).then(h=>document.getElementById('bodyArca').innerHTML=h);
}
function seleccionarFactura(id,n,m) {
    if(document.querySelector(`input[value="${id}"]`)) return;
    agregarFacturaUI(id,n,m);
    bootstrap.Modal.getInstance(document.getElementById('modalArca')).hide();
}
function agregarFacturaUI(id,n,m) {
    document.getElementById('msgNoFacturas').classList.add('d-none');
    document.getElementById('containerFacturas').innerHTML += `<div class='d-flex justify-content-between border p-1 mb-1 bg-white small item-factura'><span>Fact ${n}</span><span class='text-success'>$ ${fmtM(m)}</span><button type='button' class='btn btn-link text-danger p-0' onclick='removerFactura(this)'>x</button><input type='hidden' name='facturas_arca[]' value='${id}'><input type='hidden' class='val-factura' value='${m}'></div>`;
    validarFacturas();
}
function removerFactura(btn) { btn.closest('.item-factura').remove(); validarFacturas(); }
function validarFacturas() {
    let t=0; document.querySelectorAll('.val-factura').forEach(i=>t+=parseFloat(i.value));
    document.getElementById('lblTotalFact').innerText='$ '+fmtM(t);
    let n = parseM(document.getElementById('inputNeto').value);
    document.getElementById('lblAlertaFact').classList.toggle('d-none', Math.abs(n-t)<10);
}

// --- UTILS & FORMATO EN VIVO ---
...
function formatLive(input) {
    ...
    // 2. Parsear como entero y dividir por 100 para decimales
