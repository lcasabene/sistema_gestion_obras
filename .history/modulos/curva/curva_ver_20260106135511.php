...
// ORDEN: Anticipo (0) primero, resto (1) despues.
...
// --- SOLUCIÓN DUPLICIDAD: ARRAY DE REDETERMINACIONES USADAS ---
// Esto evitará que una misma redeterminación se muestre en dos filas distintas del mismo mes.
$redetIdsUsados = []; 
?>
            ...
            <table class="table table-bordered table-hover mb-0 table-vcenter text-center">
                ...
                <tbody>
                    <?php foreach($itemsCurva as $item): 
                        ...
                        // 1. Obtener candidatos de redeterminación para este mes
                        $candidatosRedet = $mapaCertificados[$per]['REDETERMINACION'] ?? [];
                        
                        // 2. Filtrar: Solo tomamos las que NO han sido usadas en filas anteriores
                        foreach($candidatosRedet as $cand) {
                            if(!in_array($cand['id'], $redetIdsUsados)) {
                                $certsRedet[] = $cand;
                                $redetIdsUsados[] = $cand['id']; // Marcar como usada para que la siguiente fila no la tome
                            }
                        }
