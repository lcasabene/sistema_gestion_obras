...
<script>
// Función para la herramienta de carga masiva dentro del modal
    function aplicarRangoModal() {
        ...
        if (desde > hasta) {
            alert("El mes 'Desde' no puede ser mayor al mes 'Hasta'.");
            return;
        }

        // 2. Iterar y aplicar
        for (let m = desde; m <= hasta; m++) {
            // Buscamos el input por su ID (que agregamos en el HTML arriba: input-mes-X)
            const input = document.getElementById('input-mes-' + m);
            if (input) {
                input.value = valor;
                // Disparamos la actualización visual de la barra
