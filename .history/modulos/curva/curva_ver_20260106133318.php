...
// CORRECCIÓN 2: El 'CASE' fuerza a que cualquier concepto con 'Anticipo' tenga prioridad 0 (primero)
// y el resto prioridad 1. Luego ordena por ID.
            ...
            <table class="table table-bordered table-hover mb-0 table-vcenter text-center">
                ...
                <tbody>
                    <?php foreach($itemsCurva as $item): 
                        ...
                        // CORRECCIÓN 3: AISLAMIENTO ESTRICTO
                        // Inicializamos los arrays vacíos para asegurar limpieza en cada vuelta
                        $certsBasicos = [];
                        $certsAnticipo = [];
                        $certsRedet = []; // Importante: resetear esto también

                        if ($esPlanAnticipo) {
                            // Si es renglón de Anticipo, SOLO mostramos lo que sea tipo ANTICIPO
                            $certsAnticipo = $mapaCertificados[$per]['ANTICIPO'] ?? [];
                            // Dejamos $certsBasicos y $certsRedet VACÍOS intencionalmente.
                            // Esto evita que una redet de obra se muestre en el renglón del anticipo.
                        } else {
                            // Si es renglón Normal (Certificado), mostramos ORDINARIO y REDETERMINACION
                    ...
                    <tr>
                        ...
                        <td class="col-action-redet">
                            <?php if(!$esPlanAnticipo): // Botones de Redet solo en renglones ordinarios ?>
