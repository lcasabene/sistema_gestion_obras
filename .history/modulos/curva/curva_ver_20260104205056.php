...
function calcDeducciones() {
    // Usamos parseM para leer los campos formateados (con puntos de miles)
    let bruto = parseM(document.getElementById('inputMontoBruto').value);
    let tipo = document.getElementById('modalTipo').value;
    
    // Si estoy editando el monto a mano, recalculo el % de avance
    ...
    // IMPORTANTE: Guardamos el neto en formato máquina (punto decimal, sin separador de miles)
...
function recalcularFuentes() {
    // IMPORTANTE: Leemos inputNeto como float directo (ya tiene punto decimal)
    ...
    document.querySelectorAll('#containerFufi .row').forEach(row => {
        ...
        // Mostramos el monto formateado lindo para el usuario
...
function bindMasks() { 
    document.querySelectorAll('.monto').forEach(el => { 
        // Removemos listeners previos para evitar duplicados que cuelguen el navegador
