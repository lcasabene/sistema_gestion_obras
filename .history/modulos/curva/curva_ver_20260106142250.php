...
foreach ($certificadosDB as $c) {
    ...
    if (!empty($c['curva_item_id'])) {
        // Si tiene ID, va a su dueño específico
        $mapaPorItemId[$c['curva_item_id']][$c['tipo']][] = $c;
    } else {
        // Si NO tiene ID (dato viejo o huérfano), va al backup por fecha
...
foreach ($itemsCurva as $i) {
    ...
    if ($ultimoPeriodoReal !== null && $per <= $ultimoPeriodoReal) {
        ...
        if (!in_array($per, $mesesProcesadosReal)) {
            // Para el gráfico sumamos TODO lo que haya en la fecha (con o sin ID), porque el gráfico es temporal global
            // Buscamos en ambos mapas para asegurar integridad visual del avance
            $todosCerts = [];
            // Agregar del fallback
            if(isset($mapaPorFechaFallback[$per]['ORDINARIO'])) $todosCerts = array_merge($todosCerts, $mapaPorFechaFallback[$per]['ORDINARIO']);
            // Agregar de los vinculados (hay que buscar en todo el array, es costoso pero seguro)
            ...
            <table class="table table-bordered table-hover mb-0 table-vcenter text-center">
                ...
                <tbody>
                    <?php foreach($itemsCurva as $item): 
                        ...
                        // ESTRATEGIA:
                        // 1. Buscamos por ID explícito (Prioridad Total)
                        if (isset($mapaPorItemId[$itemId])) {
                            ...
                            // 2. Fallback (Solo lo que quedó huérfano en el mapa de fechas)
