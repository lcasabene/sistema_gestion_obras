...
// 3. Certificados Reales (Ordenados por fecha y número)
...
function abrirModalNew(tipo, periodo, planPct) {
    ...
    document.getElementById('inputNroCert').value = ""; // Auto
    
    if(tipo==='ORDINARIO' || tipo==='ANTICIPO') {
        // Habilitamos el input de % también para anticipo
    ...
    if(tipo==='ANTICIPO') {
        document.getElementById('inputAnticipoPct').value = 0; // No se descuenta anticipo del anticipo
    }
...
function abrirModalEdit(id) {
    ...
    .then(data => {
        ...
        // Mostrar campo % tanto en Ordinario como Anticipo
...
function calcDeducciones() {
    ...
    // Si cambio el monto bruto manualmente, recalcular el % de avance inverso
    ...
    if(tipo !== 'ANTICIPO') {
        ...
        // En anticipo forzamos 0 descuento visual
        // Pero no cambiamos el inputAnticipoPct si ya estaba cargado, solo el monto a 0
    }
    ...
    // RECALCULAR FUENTES AUTOMÁTICAMENTE
    recalcularFuentes();
    validarFacturas();
}
...
function bindMasks() { document.querySelectorAll('.monto').forEach(el => { el.addEventListener('blur', function(){this.value=fmtM(parseM(this.value))}); el.addEventListener('focus', function(){this.value=parseM(this.value)}); }); }
</script>
