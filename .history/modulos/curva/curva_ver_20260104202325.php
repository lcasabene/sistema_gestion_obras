...
function abrirModalNew(tipo, periodo, planPct) {
    ...
    if(tipo==='ORDINARIO' || tipo==='ANTICIPO') {
        ...
        if(tipo==='ORDINARIO') document.getElementById('inputAvance').value = planPct;
    }
...
function calcDeducciones() {
    ...
    // 2. Dev. Anticipo (SI ES ANTICIPO, NO DESCUENTA)
    let ant = 0;
    if(tipo !== 'ANTICIPO') {
...
function renderFuentes(lista) {
    ...
    lista.forEach(f => {
        ...
        div.innerHTML += `<div class="row mb-1 g-1 align-items-center small"><div class="col-7">${cod} ${f.nombre} (${f.porcentaje}%)<input type="hidden" name="fuente_id[]" value="${f.fuente_id}"><input type="hidden" name="fuente_pct[]" value="${f.porcentaje}"></div><div class="col-5"><input type="text" name="fuente_monto[]" class="input-fufi monto form-control form-control-sm" value="${montoShow}"></div></div>`;
    });
    bindMasks();
}
function recalcularFuentes() {
    // CORRECCIÓN: Usar parseFloat directo porque inputNeto ya viene con formato máquina (punto decimal)
    ...
    document.querySelectorAll('#containerFufi .row').forEach(row => {
        ...
        // Calculamos el monto proporcional
        let monto = neto * (pct/100);
        
        // Asignamos al input visual usando fmtM para que se vea bonito (45.000.000,00)
        let inputMonto = row.querySelector('input[name="fuente_monto[]"]');
        inputMonto.value = fmtM(monto);
    });
}
