...
// ORDEN: Forzamos 'Anticipo' primero (0), resto (1).
...
foreach ($certificadosDB as $c) {
    ...
    // Detectar último periodo con datos reales
...
foreach ($itemsCurva as $i) {
    // EXCLUSIÓN: Saltamos el anticipo del gráfico
    ...
    if ($ultimoPeriodoReal !== null && $per <= $ultimoPeriodoReal) {
        ...
        if (!in_array($per, $mesesProcesadosReal)) {
            // Sumamos solo Ordinarios al % Físico
            ...
            <table class="table table-bordered table-hover mb-0 table-vcenter text-center">
                ...
                <tbody>
                    <?php foreach($itemsCurva as $item): 
                        ...
                        // Si es Anticipo -> Solo mostramos ANTICIPO. 
                        // Ignoramos REDETERMINACION aquí para que no se duplique con el certificado ordinario.
                        // (Si necesitas ajustar el monto del anticipo, se edita el registro de Anticipo directamente)
                        
                        if ($esPlanAnticipo) {
                            $certsAnticipo = $mapaCertificados[$per]['ANTICIPO'] ?? [];
                            $certsBasicos = []; // Vacío forzado
                            $certsRedet = [];   // Vacío forzado (Esto evita el problema de duplicidad)
                        } else {
                            // Si es Certificado Ordinario -> Mostramos ORDINARIO y REDETERMINACION
                            $certsAnticipo = []; // Vacío forzado
                    ...
                    <tr>
                        ...
                        <td class="col-action-redet">
                            <?php if(!$esPlanAnticipo): // Solo permitimos agregar/editar Redet en filas normales ?>
