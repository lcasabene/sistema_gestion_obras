...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar_curva'])) {
    try {
        ...
        $sqlVer = "INSERT INTO curva_version (obra_id, fecha_creacion, observacion, es_vigente, monto_presupuesto, plazo_meses) 
                   VALUES (:obra, NOW(), 'Generada Manualmente (Curva S)', 1, :monto, :plazo)";
        $stmtVer = $pdo->prepare($sqlVer);
        $stmtVer->execute([':obra'=>$obraId, ':monto'=>$montoClean, ':plazo'=>$plazo]);
        ...
        $pdo->commit();
        $mensaje = "Curva S guardada correctamente (V.$versionId)";
...
<form method="POST" id="formCurvaGlobal">
    ...
    <div class="container-fluid px-4 my-4">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h3 class="text-primary"><i class="bi bi-graph-up-arrow"></i> Nueva Curva S: <?= htmlspecialchars($obra['denominacion']) ?></h3>
                ...
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-md-3 d-flex align-items-end">
                        <button type="button" class="btn btn-primary w-100 fw-bold shadow-sm py-2" onclick="generarGrillaBase()">
                            <i class="bi bi-bezier2"></i> GENERAR CURVA S
                        </button>
                    </div>
                </div>
...
<script>
    ...
    // --- GENERADOR CURVA S (SINUSOIDAL) ---
    function getDistribucionCurvaS(meses) {
        let distribucion = [];
        let acumuladoAnterior = 0;
        
        for (let i = 1; i <= meses; i++) {
            // Generamos ángulo de -PI/2 (inicio) a +PI/2 (fin)
            let angulo = -Math.PI / 2 + (Math.PI * i / meses);
            
            // Función Seno normalizada de 0 a 100
            let acumuladoActual = ((Math.sin(angulo) + 1) / 2) * 100;
            
            let mensual = acumuladoActual - acumuladoAnterior;
            distribucion.push(mensual);
            acumuladoAnterior = acumuladoActual;
        }
        
        // Ajuste fino para asegurar 100.00% exacto
        let suma = distribucion.reduce((a, b) => a + b, 0);
        let resto = 100 - suma;
        if(distribucion.length > 0) distribucion[distribucion.length - 1] += resto;
    ...
    function generarGrillaBase() {
        ...
        // 2. CERTIFICADOS (CURVA S)
        let fechaCursor = new Date(fechaInicio + 'T00:00:00');
        
        // Obtenemos el array de porcentajes S
        const porcentajesS = getDistribucionCurvaS(plazo);
        ...
        for(let i=1; i<=plazo; i++) {
            ...
            // Tomamos el porcentaje correspondiente a este mes
            let pctMesS = porcentajesS[i-1];

            let tr = crearFilaHTML({
                tipo: 'certificado', idx: i, concepto: 'Certificado Nº ' + i,
                fecha: periodoDia, periodoVisual: periodoStr, pctSug: pctMesS, esAnticipo: false
            });
        ...
        filas.forEach((tr) => {
            ...
            if (tipo === 'anticipo') {
                if (usarRedetAnticipo) {
                    // CORRECCIÓN: Historia previa + Mes propio
    ...
    function actualizarFooter() {
        ...
        document.querySelectorAll('#gridBody tr').forEach(tr => {
            let tipo = tr.getAttribute('data-tipo');
            if(tipo !== 'anticipo') sumPct += parseFloat(tr.querySelector('.hidden-pct').value) || 0;
            
            sumBase += parseFloat(tr.querySelector('.hidden-bruto').value) || 0;
            sumRedet += parseFloat(tr.querySelector('.hidden-redet').value) || 0;
            sumRecup += parseFloat(tr.querySelector('.hidden-recupero').value) || 0;
            sumNeto += parseFloat(tr.querySelector('.hidden-neto').value) || 0;
        });
