...
// 1. Monto: Aseguramos que se lean como números decimales (float) para evitar errores de texto
$montoOriginal = (float)($obra['monto_original'] ?? 0);
$montoActualizado = (float)($obra['monto_actualizado'] ?? 0);

// Lógica: Si hay un monto actualizado mayor a 0, lo usamos (es el vigente).
// Si no, usamos el original.
if ($montoActualizado > 0) {
    $valMonto = $montoActualizado;
} else {
...
$valAnticipoPct = (!empty($obra['anticipo_pct'])) ? (float)$obra['anticipo_pct'] : 10;
