...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar_curva'])) {
    try {
        ...
        $stmtVer->execute([':obra'=>$obraId, ':monto'=>$montoClean, ':plazo'=>$plazo, ':mesbase' => $_POST['mes_base'] // <--- NUEVO CAMPO]);
...
<script>
    ...
    document.querySelectorAll('#gridBody tr').forEach(tr => {
        ...
        // Lógica clave:
        // El Anticipo Financiero tiene Redeterminación (se cobra),
        // pero NO suma al "Avance Físico" ni al "Monto Base del Contrato".
        if(tipo !== 'anticipo') {
            sumPct += valPct;
            sumBase += valBase;
        }
        
        // La redeterminación SÍ se suma siempre (incluida la del anticipo)
        sumRedet += valRedet;
        sumRecup += valRecup;
        sumNeto += valNeto;
    });
