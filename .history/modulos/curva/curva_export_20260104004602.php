...
// Traemos todos los detalles necesarios para identificar cada renglón
$sqlReal = "
    SELECT 
        c.id, c.periodo, c.nro_certificado, c.tipo, c.fri,
        c.monto_basico, c.monto_redeterminado, c.avance_fisico_mensual,
        -- Concatenamos facturas si hubiera más de una por certificado
    ...
    GROUP BY c.id
    ORDER BY c.tipo ASC -- Para que el ORDINARIO (Básico) salga primero si coinciden
";
$stmtC = $pdo->prepare($sqlReal);
$stmtC->execute([$cabecera['obra_real_id']]);
$allCerts = $stmtC->fetchAll(PDO::FETCH_ASSOC);

// Agrupar certificados por Periodo (YYYY-MM)
$certsByPeriod = [];
...
<head>
    ...
    <style>
        ...
        .bg-basico { background-color: #d1e7dd; } /* Verde suave */
        .bg-redet { background-color: #fff3cd; } /* Amarillo suave */
        .bg-anticipo { background-color: #cfe2ff; } /* Azul suave */
...
<table>
    ...
    <tbody>
        ...
        foreach($itemsPlan as $plan): 
            ...
            // Buscar Certificados Reales para este periodo
            $certificados = $certsByPeriod[$per] ?? [];
            
            // Determinar cuántas filas ocupará este periodo (Mínimo 1 para mostrar el Plan)
            $cantFilas = max(1, count($certificados));
            
            // ITERAR SOBRE LAS FILAS DE ESTE PERIODO
            for ($i = 0; $i < $cantFilas; $i++):
                $cert = $certificados[$i] ?? null;
                
                // Estilos y Datos del Certificado
                $bgClass = "";
                $tipoTxt = "-";
                $nroTxt = "-";
                $friTxt = "-";
                ...
                if ($cert) {
                    ...
                    if ($cert['tipo'] == 'ORDINARIO') {
                        ...
                        $acumReal += $cert['avance_fisico_mensual']; // Sumar al acumulado global
                    } 
                ...
                // Cálculo Desvío (Solo en la fila donde cambia el acumulado real, usualmente la del básico)
                // Para simplificar en Excel, mostramos desvío acumulado en cada línea de periodo o solo en la primera
                $desvioTxt = "-";
                $acumRealTxt = "-";
                ...
                if ($i === 0 && !$esAnticipoPlan) {
                    ...
                    // Solo mostramos desvío si ya hubo movimiento real o si ya pasó el tiempo
                    if ($acumReal > 0) {
                ...
                // Borde superior fuerte si es la primera fila del periodo (para separar meses visualmente)
        ...
        <?php 
            endfor; // Fin for filas del periodo
        endforeach; // Fin items plan
        ?>
    </tbody>
</table>
