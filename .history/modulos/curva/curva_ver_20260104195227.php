                ...
                <div class="modal-body">
                    ...
                    <div class="row mb-3 bg-light p-2 rounded mx-0 border">
                        ...
                        <div class="col-md-6 div-ordinario d-none">
                            <label class="form-label small fw-bold" id="lblInputAvance">% Avance / Financiero</label>
...
function abrirModalEdit(id) {
    ...
    .then(data => {
        ...
        // Forzar formato correcto para evitar discrepancias
        ...
        // 2. Cargar Fuentes y formatearlas
...
function calcDeducciones() {
    ...
    // Reverse calc del porcentaje si edito monto a mano
    ...
    recalcularFuentes(); // CLAVE: Recalcular fuentes al cambiar valores
}
...
function renderFuentes(lista) {
    ...
    div.innerHTML = '';
    lista.forEach(f => {
        // Aseguramos que el monto venga formateado
        let montoShow = (typeof f.monto_asignado === 'number') ? fmtM(f.monto_asignado) : f.monto_asignado; 
        if(!montoShow) montoShow = "0,00";

        div.innerHTML += `<div class="row mb-1 g-1 align-items-center small"><div class="col-7">${f.nombre} (${f.porcentaje}%)<input type="hidden" name="fuente_id[]" value="${f.fuente_id}"><input type="hidden" name="fuente_pct[]" value="${f.porcentaje}"></div><div class="col-5"><input type="text" name="fuente_monto[]" class="input-fufi monto" value="${montoShow}"></div></div>`;
    });
    bindMasks();
}
...
// ARCA... (Mismo código que tenias, omitido por brevedad, está OK en tu versión anterior)
...
function agregarFacturaUI(id,n,m) {
    ...
    validarFacturas(); // Corregido nombre funcion
}
function removerFactura(btn) { btn.closest('.item-factura').remove(); }
...
// NUEVO: Formato en vivo mientras escribes
function formatLive(input) {
    // 1. Guardar posición del cursor
    let cursor = input.selectionStart;
    let originalLen = input.value.length;
    
    // 2. Limpiar todo lo que no sea numero
    let raw = input.value.replace(/\D/g, ''); 
    if(!raw) return;
    
    // 3. Convertir a decimal (asumimos ultimos 2 son decimales si escribe rapido, o logica simple)
    // Mejor enfoque simple: Parsear como entero y formatear
    // Pero para inputs con comas es complejo.
    // Usaremos el enfoque simple de parseM/fmtM solo si el usuario no está borrando
    
    // NOTA: Para no complicar con saltos de cursor, usamos una logica simple:
    // Solo formateamos al perder foco (ya está en bindMasks).
    // Si quieres formato estricto en vivo:
    let val = parseFloat(raw) / 100; // Asume escritura de centavos
    // input.value = fmtM(val); // Esto a veces molesta al usuario si no está acostumbrado
}
