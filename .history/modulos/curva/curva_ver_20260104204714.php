            ...
            <table class="table table-bordered table-hover mb-0 table-vcenter text-center">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="col-basico">
                             <?php if(!empty($certsBasicos)): foreach($certsBasicos as $cb): ?><div class="small fw-bold mb-1">$ <?= fmtM($cb['monto_basico']) ?></div><?php endforeach; elseif(!empty($certsAnticipo)): ?><div class="small fw-bold">$ <?= fmtM($certsAnticipo[0]['monto_basico']) ?></div><?php endif; ?>
                        </td>
            ...
            <form action="../certificados/certificados_guardar_modal.php" method="POST" id="formModal">
                ...
                <div class="modal-body">
                    <div class="row align-items-center mb-3">
                        <div class="col-md-6"><div class="alert alert-secondary py-1 small mb-0 fw-bold"><i class="bi bi-info-circle"></i> Obra: $ <?= fmtM($cabecera['monto_actualizado']) ?></div></div>
...
function abrirModalNew(tipo, periodo, planPct) {
    ...
    if(tipo === 'ANTICIPO') {
        document.getElementById('inputAnticipoPct').value = 0; // El anticipo no se descuenta a sí mismo
    }
...
function abrirModalEdit(id) {
    ...
    .then(data => {
        ...
        // Renderizar Fuentes y Facturas antes de calcular para que los inputs existan
...
function calcDeducciones() {
    ...
    // Si edito Bruto, recalculo el % avance
    ...
    // 2. Dev. Anticipo (Si es Anticipo Financiero, NO se descuenta)
    let ant = 0;
    if(tipo !== 'ANTICIPO') {
    ...
    // Guardamos el neto en formato máquina (ej: 31000000.00)
...
function recalcularFuentes() {
    // CORRECCIÓN CRÍTICA: Usamos parseFloat porque inputNeto ya tiene punto decimal (viene de toFixed)
    // NO USAR parseM AQUÍ
...
function renderFuentes(lista) {
    ...
    lista.forEach(f => {
        ...
        // CORRECCIÓN: Validar que monto_asignado sea numérico antes de formatear
        let valor = (f.monto_asignado !== undefined && f.monto_asignado !== null) ? f.monto_asignado : 0;
        ...
        div.innerHTML += `
            <div class="row mb-1 g-1 align-items-center small">
                ...
                <div class="col-5">
                    <input type="text" name="fuente_monto[]" class="input-fufi monto form-control form-control-sm" value="${fmtM(valor)}">
                </div>
            </div>`;
    });
    bindMasks();
...
// UTILS
function parseM(v) { 
    // Limpieza agresiva: Quita puntos de mil, cambia coma decimal por punto
    if (!v) return 0;
...
function bindMasks() { 
    document.querySelectorAll('.monto').forEach(el => { 
        // Clonar nodo para evitar duplicar eventos (opcional pero seguro) o simplemente agregar
...
function onFocusMonto(){ 
    ...
    this.value = (val === 0) ? '' : val; // Limpiar si es 0 para escribir fácil
}

// --- LOGICA FACTURAS (Opcional, mantener si la usas) ---
function abrirModalArca() { /* ... tu codigo de facturas ... */ }
function validarFacturas() { /* ... tu codigo de facturas ... */ }
function agregarFacturaUI(id, num, monto) { /* ... tu codigo de facturas ... */ }
</script>