...
// CORRECCIÓN 2: Agregamos orden por ID para que si las fechas son iguales, respete el orden de creación (Anticipo primero)
$sqlItems = "SELECT * FROM curva_items WHERE version_id = ? ORDER BY periodo ASC, id ASC";
...
// --- CORRECCIÓN 1: Lógica del Gráfico ---
$labels = []; 
$dataPlan = []; 
$dataReal = []; 
$acumPlan = 0; 
...
// Array auxiliar para no sumar dos veces el real del mismo mes
$mesesProcesadosReal = []; 

foreach ($itemsCurva as $i) {
    // Solo graficamos items que NO son puramente financieros (aunque el anticipo a veces se muestra, en curva S física suele excluirse o mantenerse en 0)
    // Asumiremos que queremos ver la curva de avance FÍSICO.
    
    $per = date('Y-m', strtotime($i['periodo']));
    $labelPer = date('m/y', strtotime($i['periodo']));
    
    // Si hay dos renglones para el mismo mes (ej: Oct), solo agregamos el label una vez o lo repetimos según prefieras. 
    // Para curva S standard, suele ser un punto por mes. Aquí seguiremos la tabla.
    $labels[] = $labelPer;
    ...
    // LOGICA REAL:
    // Solo sumamos el avance real SI NO lo hemos sumado ya para este periodo en esta iteración de la gráfica
    // OJO: Si el certificado real incluye avance físico, lo sumamos.
    
    $avanceMes = 0; 
    $huboDato = false;

    // Verificamos si este mes ya tiene datos reales procesados en el acumulado para evitar duplicados visuales
    // (A menos que tu lógica permita múltiples certificados ordinarios que suman al mes, lo cual es correcto, 
    // pero aquí el problema era que el LOOP pasaba 2 veces por el mismo mes).
    
    // Truco: Solo leemos certificados ORDINARIOS para el avance físico Real.
    if(isset($mapaCertificados[$per]['ORDINARIO'])) {
        // Solo sumamos si este Item de Planificación NO es el Anticipo Financiero (para no duplicar la lectura del mes)
        // O si es el Anticipo, asumimos que no hay avance físico asociado.
        
        $esAnticipoPlan = (stripos($i['concepto'], 'anticipo') !== false);
        
        // Si es un renglón de plan de obra (no anticipo), sumamos lo real
        if (!$esAnticipoPlan && !in_array($per, $mesesProcesadosReal)) {
            foreach($mapaCertificados[$per]['ORDINARIO'] as $cert) {
                $avanceMes += $cert['avance_fisico_mensual'];
            }
            $mesesProcesadosReal[] = $per; // Marcamos mes como sumado
            $huboDato = true;
        }
    }
    
    // Si ya hay acumulado o hubo dato este mes, mostramos punto. Si no, null (corta la linea) o repetimos el anterior.
    if ($huboDato || $acumReal > 0) { 
        $acumReal += $avanceMes; 
        $dataReal[] = $acumReal; 
    } else { 
        $dataReal[] = null; 
            ...
            <table class="table table-bordered table-hover mb-0 table-vcenter text-center">
                ...
                <tbody>
                    <?php foreach($itemsCurva as $item): 
                        ...
                        // CORRECCIÓN 2: Filtro estricto para evitar duplicados en la visualización
                        // Si el renglón es Anticipo, SOLO buscamos certificados tipo ANTICIPO
                        // Si el renglón es Normal, SOLO buscamos certificados tipo ORDINARIO
                        ...
                        if ($esPlanAnticipo) {
                            ...
                            // Dejamos $certsBasicos vacío a propósito
                        } else {
                            $certsBasicos = $mapaCertificados[$per]['ORDINARIO'] ?? [];
                            // Dejamos $certsAnticipo vacío a propósito
                        }
                    ...
                    <tr>
                        ...
                        <td class="text-start small fst-italic text-muted"><?= utf8_decode($item['concepto']) ?></td>
                        ...
                        <td class="col-basico">
                            <?php 
                            if($esPlanAnticipo && !empty($certsAnticipo)): 
                                foreach($certsAnticipo as $ca): ?><span class="nro-cert">#<?= $ca['nro_certificado'] ?></span><?php endforeach; 
                            elseif(!empty($certsBasicos)): 
                                foreach($certsBasicos as $cb): ?><div class="mb-1"><span class="nro-cert">#<?= $cb['nro_certificado'] ?></span></div><?php endforeach; 
                            else: ?> - <?php endif; ?>
                        </td>
                        <td class="col-basico">
                            <?php 
                        ...
                        <td class="col-redet">
                            <?php if(!$esPlanAnticipo && !empty($certsRedet)): foreach($certsRedet as $cr): echo "<div class='text-fri mb-1'>".fmtFri($cr['fri'])."</div>"; endforeach; endif; ?>
                        </td>
                        <td class="col-redet">
                            <?php if(!$esPlanAnticipo && !empty($certsRedet)): foreach($certsRedet as $cr): echo "<div class='small fw-bold text-dark mb-1'>$ ".fmtM($cr['monto_redeterminado'])."</div>"; endforeach; endif; ?>
                        </td>
                ...
                <div class="modal-body">
                    ...
                    <div class="row mb-3 mx-0">
                        <div class="col-12 border-bottom mb-2 pb-1 d-flex justify-content-between align-items-center">
                            ...
                            <button type="button" class="btn btn-outline-primary btn-sm py-0" onclick="recalcularFuentes(true)">
                                <i class="bi bi-calculator"></i> Aplicar Pari Passu
                            </button>
                        </div>
                        <div class="col-12" id="containerFufi"></div>
                    </div>
                    <div class="alert alert-primary py-2 d-flex justify-content-between align-items-center mb-0"><span class="fw-bold">NETO A PAGAR:</span><h4 class="mb-0 fw-bold" id="lblNeto">$ 0,00</h4><input type="hidden" name="monto_neto" id="inputNeto"></div>
                </div>
...
new Chart(ctx, { 
    ...
    options: { maintainAspectRatio: false, scales: { y: { beginAtZero: false, grace: '5%' } }, plugins: { tooltip: { callbacks: { label: function(context) { return context.dataset.label + ': ' + context.parsed.y.toLocaleString('es-AR') + '%'; } } } } } 
});
...
function abrirModalNew(tipo, periodo, planPct) {
    ...
    // Cargar fuentes y calcular automáticamente al INICIAR uno nuevo
...
function calcDeducciones() {
    ...
    // CORRECCIÓN 3: NO llamamos a recalcularFuentes() aquí. 
    // Si el usuario ya editó manualmente una fuente, esto lo borraba.
    // El usuario debe pulsar "Aplicar Pari Passu" o editar manualmente.
    // Solo lo llamamos si es un nuevo certificado y nunca se editó, pero es más seguro dejar que el usuario lo pida.
}

// CORRECCIÓN 3: Pari Passu sobre BRUTO y control manual
function recalcularFuentes(force = false) {
    // Calculamos sobre el BRUTO (antes era Neto)
