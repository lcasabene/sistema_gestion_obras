...
// Datos Gráfico (Preparación segura)
...
foreach ($itemsCurva as $i) {
    if (stripos($i['concepto'], 'anticipo') === false) {
        ...
        if ($hubo || $acumReal > 0) { $acumReal += $avanceMes; $dataReal[] = $acumReal; } else { $dataReal[] = null; }
    }
}
?>
...
<head>
    ...
    <style>
        ...
        /* MODIFICADO: Columnas compactas para planificación */
        .col-plan { 
            background-color: #f8f9fa; 
            border-right: 1px solid #dee2e6;
            font-size: 0.8rem; /* Letra más chica */
            padding: 4px !important; /* Menos padding */
            white-space: nowrap;
            width: 1%; /* Forza al ancho mínimo del contenido */
        }
        ...
        /* NUEVO: Columnas de acción separadas */
            ...
            <table class="table table-bordered table-hover mb-0 table-vcenter text-center">
                <thead class="table-dark text-uppercase small">
                    ...
                    <tr>
                        ...
                        <th>% Real</th>
                        <th>Monto</th>
                        <th class="bg-white text-dark"><i class="bi bi-gear"></i></th> <th>FRI</th>
                        <th>Monto</th>
                        <th class="bg-white text-dark"><i class="bi bi-gear"></i></th> </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="col-basico">
                            <?php if($esAnticipo && !empty($certsAnticipo)): foreach($certsAnticipo as $ca): ?><span class="nro-cert">#<?= $ca['nro_certificado'] ?></span><?php endforeach; elseif(!empty($certsBasicos)): foreach($certsBasicos as $cb): ?><div class="mb-1"><span class="nro-cert">#<?= $cb['nro_certificado'] ?></span></div><?php endforeach; else: ?> - <?php endif; ?>
                        </td>
                        <td class="col-basico">
                        ...
                        <td class="col-basico">
                            <?php if($esAnticipo && !empty($certsAnticipo)): ?><div class="small fw-bold">$ <?= fmtM($certsAnticipo[0]['monto_bruto']) ?></div><?php elseif(!empty($certsBasicos)): foreach($certsBasicos as $cb): ?><div class="small fw-bold mb-1">$ <?= fmtM($cb['monto_basico']) ?></div><?php endforeach; else: ?> - <?php endif; ?>
                        </td>
                        ...
                        <td class="col-redet"><?php if(!empty($certsRedet)): foreach($certsRedet as $cr): echo "<div class='text-fri mb-1'>".fmtFri($cr['fri'])."</div>"; endforeach; endif; ?></td>
        ...
        <div class="modal-content">
            ...
            <form action="../certificados/certificados_guardar_modal.php" method="POST" id="formModal">
                ...
                <div class="modal-body">
                    ...
                    <div class="row mb-3 mx-0">
                        <div class="col-12 border-bottom mb-2 pb-1 d-flex justify-content-between align-items-center"><span class="text-muted small text-uppercase fw-bold">3. Fuentes de Financiamiento</span><button type="button" class="btn btn-link btn-sm py-0" onclick="recalcularFuentes()">Autocalcular</button></div>
...
<script>
// --- CORRECCIÓN CRÍTICA PARA CHART.JS ---
// Usamos json_encode para asegurar que PHP pase un número limpio (ej: 1500.50) y no una cadena con comas (1.500,50)
