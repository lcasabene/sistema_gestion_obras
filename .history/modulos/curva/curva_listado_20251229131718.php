...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_indices') {
    try {
        ...
        // SQL corregido con ON DUPLICATE KEY UPDATE
        ...
        // AQUÍ ESTABA EL ERROR: Aseguramos que la variable se llame $datoInput
        foreach ($_POST['indices'] as $mes => $datoInput) {
            
            // 1. Limpieza: Si viene vacío, ponemos 0
            if ($datoInput === '' || $datoInput === null) {
                $datoInput = 0;
            }

            // 2. Limpieza: Cambiar coma por punto (para evitar errores decimales)
            // Usamos $datoInput, no $valor (que era lo que daba error)
