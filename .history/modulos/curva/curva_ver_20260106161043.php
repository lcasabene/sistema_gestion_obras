...
// 2. Planificación (Items) - ANTICIPO PRIMERO
...
// IMPORTANTE: Traemos 'curva_item_id' para hacer el match exacto
...
// --- LÓGICA DE MAPEO (EL CORAZÓN DE LA SOLUCIÓN) ---
$mapaPorItemId = [];       // Aquí guardamos los que tienen dueño (ID)
$mapaPorFechaFallback = [];// Aquí los huérfanos (para compatibilidad vieja)
$ultimoPeriodoReal = null; 

foreach ($certificadosDB as $c) {
    $periodo7 = substr($c['periodo'], 0, 7);
    if ($ultimoPeriodoReal === null || $periodo7 > $ultimoPeriodoReal) {
        $ultimoPeriodoReal = $periodo7;
    }

    if (!empty($c['curva_item_id'])) {
        // Si tiene ID, va directo a su cajita
        $mapaPorItemId[$c['curva_item_id']][$c['tipo']][] = $c;
    } else {
        // Si no, va al montón de la fecha (fallback)
        $mapaPorFechaFallback[$periodo7][$c['tipo']][] = $c;
    }
}
...
// --- GRÁFICO ---
$labels = []; $dataPlan = []; $dataReal = []; $acumPlan = 0; $acumReal = 0; $mesesProcesadosReal = []; 

foreach ($itemsCurva as $i) {
    if (stripos($i['concepto'], 'anticipo') !== false) continue; // Saltar anticipo en gráfico
    ...
    if ($ultimoPeriodoReal !== null && $per <= $ultimoPeriodoReal) {
        ...
        if (!in_array($per, $mesesProcesadosReal)) {
            // Recolectar avance de todos los ordinarios del mes (con o sin ID)
            $todosCerts = $mapaPorFechaFallback[$per]['ORDINARIO'] ?? [];
            foreach($mapaPorItemId as $pid => $types) {
                if(isset($types['ORDINARIO'])) {
                    foreach($types['ORDINARIO'] as $co) {
                        if(substr($co['periodo'],0,7) === $per) $todosCerts[] = $co;
                    }
                }
            }
            foreach($todosCerts as $cert) $avanceMes += $cert['avance_fisico_mensual'];
...
<head>
    ...
    <style>
        ...
        /* Debug visual pequeño para validar IDs */
            ...
            <table class="table table-bordered table-hover mb-0 table-vcenter text-center">
                ...
                <tbody>
                    <?php foreach($itemsCurva as $item): 
                        ...
                        // 1. Prioridad: ¿Tenemos datos vinculados a ESTE item ID específico?
                        if (isset($mapaPorItemId[$itemId])) {
                            $certsBasicos = $mapaPorItemId[$itemId]['ORDINARIO'] ?? [];
                            $certsAnticipo = $mapaPorItemId[$itemId]['ANTICIPO'] ?? [];
                            $certsRedet = $mapaPorItemId[$itemId]['REDETERMINACION'] ?? [];
                        } else {
                            // 2. Fallback: Si no hay nada por ID, buscamos por fecha (para datos viejos)
                            // OJO: Aquí se mezclaban antes. Ahora solo entramos si NO hay ID.
                            $posibles = $mapaPorFechaFallback[$per] ?? [];
                            if ($esPlanAnticipo) {
                                $certsAnticipo = $posibles['ANTICIPO'] ?? [];
                                // IMPORTANTE: Si estamos en modo fecha (viejo), no mostramos redet en anticipo para evitar duplicidad visual
                            } else {
                                $certsBasicos = $posibles['ORDINARIO'] ?? [];
                                $certsRedet = $posibles['REDETERMINACION'] ?? [];
                            }
                        }
                        ...
                        <td class="col-action-basico border-end">
                            <?php if($esPlanAnticipo): ?>
                                <?php if(empty($certsAnticipo)): ?>
                                    <button class="btn btn-sm btn-outline-primary py-0 px-1" title="Cargar Anticipo" onclick="abrirModalNew('ANTICIPO', '<?= $per ?>', 0, <?= $itemId ?>)"><i class="bi bi-plus-lg"></i></button>
                                <?php else: ?>
                                    <button class="btn btn-sm btn-primary py-0 px-1" title="Editar Anticipo" onclick="abrirModalEdit(<?= $certsAnticipo[0]['id'] ?>, <?= $itemId ?>)"><i class="bi bi-pencil-fill"></i></button>
                                <?php endif; ?>
                            <?php else: ?>
                                <?php if(empty($certsBasicos)): ?>
                                    <button class="btn btn-sm btn-success py-0 px-1 shadow-sm" title="Cargar Certificado" onclick="abrirModalNew('ORDINARIO', '<?= $per ?>', <?= $planPct ?>, <?= $itemId ?>)"><i class="bi bi-plus-lg"></i></button>
                                <?php else: foreach($certsBasicos as $cb): ?>
                                    <button class="btn btn-sm btn-outline-success py-0 px-1 mb-1 d-block mx-auto" title="Editar Certificado" onclick="abrirModalEdit(<?= $cb['id'] ?>, <?= $itemId ?>)"><i class="bi bi-pencil-square"></i></button>
                        ...
                        <td class="col-action-redet">
                            <div class="d-flex justify-content-center gap-1 flex-wrap">
                                <?php if(!empty($certsRedet)): foreach($certsRedet as $cr): ?>
                                    <button class="btn btn-sm btn-warning py-0 px-1 text-dark" title="Editar Redet." onclick="abrirModalEdit(<?= $cr['id'] ?>, <?= $itemId ?>)"><i class="bi bi-pencil"></i></button>
                                <?php endforeach; endif; ?>
                                <button class="btn btn-sm btn-outline-warning text-dark py-0 px-1" title="Agregar Redet." onclick="abrirModalNew('REDETERMINACION', '<?= $per ?>', 0, <?= $itemId ?>)"><i class="bi bi-plus"></i></button>
                            </div>
                        </td>
                    </tr>
                    <?php endforeach; ?>
...
<div class="modal fade" id="modalCertificadoUpd" tabindex="-1">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="../certificados/certificados_guardar_modal.php" method="POST" id="formCertificadoUpd">
                ...
                <input type="hidden" name="curva_item_id" id="chk_modalCurvaItemId" value="">
                ...
                <div class="modal-body">
                    <div class="row mb-2">
                        <div class="col-12 text-end">
                            <label class="small text-muted me-1">Debug ID:</label>
                            <input type="text" id="debug_cert_id" class="debug-id" disabled>
                    ...
                    <div class="row mb-3 bg-light p-2 rounded mx-0 border">
                        ...
                        <div class="col-md-6 div-ordinario d-none">
                            ...
                            <div class="input-group"><input type="number" step="0.01" name="avance_fisico" id="chk_inputAvance" class="form-control fw-bold text-center" oninput="calcBasico()"><span class="input-group-text">%</span></div>
                            <small class="text-primary" id="lblPlanificado"></small>
                        </div>
                        <div class="col-md-3 div-redet d-none"><label class="form-label small fw-bold">FRI</label><input type="number" step="0.0001" name="fri" id="chk_inputFri" class="form-control text-center" placeholder="1.0000"></div>
                        <div class="col-md-6"><label class="form-label small fw-bold">Monto Bruto ($)</label><input type="text" name="monto_bruto" id="chk_inputMontoBruto" class="form-control fw-bold text-end monto" required oninput="calcDeducciones()"></div>
                    </div>
                    <div class="row mb-3 mx-0">
                        <div class="col-12 text-muted small text-uppercase fw-bold border-bottom mb-2">2. Deducciones</div>
                        <div class="col-md-4 mb-2"><label class="small">Fondo Reparo</label><div class="input-group input-group-sm mb-1"><span class="input-group-text">5%</span><div class="input-group-text bg-white"><input class="form-check-input mt-0" type="checkbox" name="fondo_reparo_sustituido" id="chk_checkSustituido" value="1" onchange="calcDeducciones()"><label class="small ms-1 mb-0" for="chk_checkSustituido">Sust.</label></div></div><input type="text" name="fondo_reparo_monto" id="chk_inputFondoReparo" class="form-control form-control-sm text-end text-danger monto" readonly></div>
                        <div class="col-md-4 mb-2"><label class="small">Dev. Anticipo</label><div class="input-group input-group-sm mb-1"><span class="input-group-text text-muted"><?= $cabecera['anticipo_pct'] ?>%</span><input type="number" step="0.01" name="anticipo_pct_aplicado" id="chk_inputAnticipoPct" class="form-control form-control-sm" value="<?= $cabecera['anticipo_pct'] ?>" oninput="calcDeducciones()"></div><input type="text" name="anticipo_descuento" id="chk_inputAnticipoMonto" class="form-control form-control-sm text-end text-danger monto" readonly></div>
                        <div class="col-md-4 mb-2"><label class="small">Multas / Otros</label><input type="text" name="multas_monto" id="chk_inputMultas" class="form-control form-control-sm text-end text-danger monto" value="0,00" oninput="calcDeducciones()"></div>
                    </div>
                    <div class="row mb-3 mx-0">
                        <div class="col-12 border-bottom mb-2 pb-1 d-flex justify-content-between align-items-center">
                            ...
                            <button type="button" class="btn btn-outline-primary btn-sm py-0" onclick="recalcularFuentes(true)"><i class="bi bi-calculator"></i> Aplicar Pari Passu</button>
                        </div>
                        <div class="col-12" id="containerFufi"></div>
                    </div>
                    <div class="alert alert-primary py-2 d-flex justify-content-between align-items-center mb-0"><span class="fw-bold">NETO A PAGAR:</span><h4 class="mb-0 fw-bold" id="lblNeto">$ 0,00</h4><input type="hidden" name="monto_neto" id="chk_inputNeto"></div>
                </div>
                <div class="modal-footer bg-light p-2"><button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Cancelar</button><button type="submit" class="btn btn-primary btn-sm px-4 fw-bold">Guardar</button></div>
            </form>
        </div>
    </div>
</div>
...
function abrirModalEdit(id, itemId) {
    ...
    .then(data => {
        ...
        if(data.tipo==='ORDINARIO' || data.tipo==='ANTICIPO') {
            ...
            if(data.tipo==='ORDINARIO') document.getElementById('chk_inputAvance').value = data.avance_fisico_mensual;
        }
...
function resetModal(tipo, periodo, id, itemId) {
    ...
    if(!f) { console.error("Formulario no encontrado: " + FORM_ID); return; }
    f.reset();
    ...
    document.getElementById('chk_modalCurvaItemId').value = itemId; 
    
    // Debug visual
    if(document.getElementById('debug_cert_id')) document.getElementById('debug_cert_id').value = id;
    if(document.getElementById('debug_link_id')) document.getElementById('debug_link_id').value = itemId;
...
function calcDeducciones() {
    ...
    if(tipo !== 'ANTICIPO') {
        ...
        let el = document.getElementById('chk_inputAnticipoPct');
        if(el) el.value = 0; 
    }
    ...
    document.getElementById('chk_inputNeto').value = neto.toFixed(2);
}

function recalcularFuentes(force = false) {
    let baseCalculo = parseM(document.getElementById('chk_inputMontoBruto').value) || 0;
