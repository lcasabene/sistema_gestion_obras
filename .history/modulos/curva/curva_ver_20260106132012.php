...
// CORRECCIÓN: Ordenamos primero por Periodo, luego forzamos ANTICIPO primero (CASE), y luego por ID.
$sqlItems = "SELECT * FROM curva_items 
             WHERE version_id = ? 
             ORDER BY periodo ASC, 
                      CASE WHEN concepto LIKE '%anticipo%' THEN 0 ELSE 1 END ASC, 
                      id ASC";
...
$mapaCertificados = [];
// Variable para determinar el último periodo con datos reales
...
foreach ($certificadosDB as $c) {
    ...
    // Guardamos la fecha más alta registrada con datos
...
foreach ($itemsCurva as $i) {
    ...
    // REAL
    // Solo procesamos real si el periodo actual es menor o igual al último periodo con datos.
    // Si no hay datos (obra nueva), ultimoPeriodoReal es null.
    
    if ($ultimoPeriodoReal !== null && $per <= $ultimoPeriodoReal) {
        $avanceMes = 0; 
        
        // Evitar sumar dos veces si hay múltiples renglones en el mismo mes
        if (!in_array($per, $mesesProcesadosReal)) {
            // Buscamos certificados ORDINARIOS para el avance físico
        ...
        // Agregamos el punto acumulado (aunque sea el mismo del mes anterior, se mantiene la linea plana hasta hoy)
        $dataReal[] = $acumReal;
    } else {
        // Futuro: Cortamos la línea
        $dataReal[] = null;
    }
}
?>
            ...
            <table class="table table-bordered table-hover mb-0 table-vcenter text-center">
                ...
                <tbody>
                    <?php foreach($itemsCurva as $item): 
                        ...
                        $certsBasicos = [];
                        $certsAnticipo = [];
                        // La redeterminación se busca siempre, por si el anticipo tiene redet
                        $certsRedet = $mapaCertificados[$per]['REDETERMINACION'] ?? [];
                        
                        // OJO: Hay que filtrar las redeterminaciones para saber si pertenecen al anticipo o al ordinario
                        // Como la tabla 'certificados' no vincula explícitamente a un item de curva,
                        // asumiremos que si hay una redet en el mes del anticipo, y este renglón es anticipo, deberíamos poder gestionarla.
                        // Sin embargo, lo usual es que la redet se cargue general para el mes.
                        // Para simplificar: mostramos todas las redets del mes en el renglón correspondiente si el usuario las carga ahí.
                        // Pero mejor: Permitir "Agregar Redet" en ambos renglones.
