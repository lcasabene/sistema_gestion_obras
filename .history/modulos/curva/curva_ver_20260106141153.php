...
// 2. Planificación (Items) - Ordenamos Anticipo primero
...
// Obtenemos también el campo 'curva_item_id'
...
// MAPEO INTELIGENTE
// Agrupamos certificados por el ID del item de curva (si existe vinculación)
// Si no existe (datos viejos sin migrar), usaremos un array de respaldo por fecha
$mapaPorItemId = [];
...
foreach ($certificadosDB as $c) {
    ...
    // Mapeo Principal: Por ID de Item
    if (!empty($c['curva_item_id'])) {
        $mapaPorItemId[$c['curva_item_id']][$c['tipo']][] = $c;
    } 
    
    // Mapeo Respaldo (para gráfico y compatibilidad)
...
foreach ($itemsCurva as $i) {
    ...
    // Para el gráfico usamos el fallback global por fecha, ya que el gráfico es temporal
    if ($ultimoPeriodoReal !== null && $per <= $ultimoPeriodoReal) {
        $avanceMes = 0; 
        if (!in_array($per, $mesesProcesadosReal)) {
            if(isset($mapaPorFechaFallback[$per]['ORDINARIO'])) {
                foreach($mapaPorFechaFallback[$per]['ORDINARIO'] as $cert) {
            ...
            <table class="table table-bordered table-hover mb-0 table-vcenter text-center">
                ...
                <tbody>
                    <?php foreach($itemsCurva as $item): 
                        ...
                        // RECUPERACIÓN DE DATOS REALES POR ID DE ITEM
                        // Esto garantiza 100% que no se mezclen anticipo y certificado aunque sean del mismo mes
                        
                        $certsBasicos = [];
                        $certsAnticipo = [];
                        $certsRedet = [];
                        
                        // ¿Tenemos certificados vinculados específicamente a este ID?
                        if (isset($mapaPorItemId[$itemId])) {
                            ...
                            // FALLBACK: Si no hay link por ID (datos viejos), intentamos adivinar por fecha y tipo
                            // Solo si NO se ejecutó el script de migración SQL
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="../certificados/certificados_guardar_modal.php" method="POST" id="formCertificadoUpd">
                ...
                <input type="hidden" name="curva_item_id" id="modalCurvaItemId" value="">
...
function abrirModalEdit(id, itemId) {
    ...
    .then(r => r.json())
    .then(data => {
        // Pasamos itemId al reset para que se mantenga en el hidden si se re-guarda
...
function resetModal(tipo, periodo, id, itemId) {
    ...
    // ASIGNAMOS EL ID VINCULADO
    document.getElementById('modalCurvaItemId').value = itemId; 
    ...
    if(pctObra) document.getElementById('inputAnticipoPct').value = pctObra.value;
}
...
function calcDeducciones() {
    ...
    if(document.activeElement.id === 'inputMontoBruto' && montoObraRef > 0 && tipo === 'ORDINARIO') {
    ...
    if(tipo !== 'ANTICIPO') {
        ...
        let el = document.getElementById('inputAnticipoPct');
        if(el) el.value = 0; 
    }
