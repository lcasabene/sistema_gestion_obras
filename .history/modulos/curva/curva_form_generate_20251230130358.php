...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar_curva'])) {
    try {
        ...
        // Limpieza monto contrato visual (quitar puntos, cambiar coma por punto)
                ...
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-md-3 d-flex align-items-end">
                        <button type="button" class="btn btn-primary w-100 fw-bold shadow-sm py-2" onclick="generarGrillaBase()">
                            <i class="bi bi-table"></i> GENERAR TABLA (Curva S)
                        </button>
                    </div>
                </div>
...
<script>
    ...
    function generarPesosCurvaS(meses) {
        // Usamos una distribución normal (Gauss) centrada en la mitad del plazo
        let pesos = [];
        let totalPeso = 0;
        const media = meses / 2;     // Centro de la campana
        const desviacion = meses / 4; // Ancho de la campana (ajustable para más o menos pendiente)

        for(let i = 1; i <= meses; i++) {
            // Fórmula densidad de probabilidad normal
            let x = i;
        ...
        let porcentajes = pesos.map(p => (p / totalPeso) * 100);
        return porcentajes;
    }
    ...
    function generarGrillaBase() {
        ...
        // 2. CERTIFICADOS (USANDO CURVA S)
        let fechaCursor = new Date(fechaInicio + 'T00:00:00');
        
        // Obtenemos el array de porcentajes distribuidos en 'S'
        let porcentajesS = generarPesosCurvaS(plazo);
        ...
        for(let i=1; i<=plazo; i++) {
            ...
            // Usamos el porcentaje de la curva S correspondiente al mes (i-1 porque array empieza en 0)
            let pctDelMes = porcentajesS[i-1];
