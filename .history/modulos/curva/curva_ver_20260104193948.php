...
$sqlCerts = "SELECT * FROM certificados WHERE obra_id = ? AND estado != 'ANULADO' ORDER BY nro_certificado ASC";
            ...
            <form action="../certificados/certificados_guardar_modal.php" method="POST" id="formModal">
                ...
                <div class="modal-body">
                    <div class="alert alert-secondary py-1 small text-center mb-3 fw-bold">
                        <i class="bi bi-info-circle"></i> Monto Obra: $ <?= fmtM($cabecera['monto_actualizado']) ?>
                    </div>
...
function abrirModalNew(tipo, periodo, planPct) {
    ...
    if(tipo==='ORDINARIO') {
        ...
        // Para anticipo, desactivamos deducciones por defecto
...
function abrirModalEdit(id) {
    ...
    .then(data => {
        ...
        // Recalcular para mostrar totales
        calcDeducciones();
...
function calcDeducciones() {
    ...
    // 1. Fondo Reparo
    let fr = 0;
    // En anticipos a veces no se retiene FR, depende la norma. Aquí asumimos que si.
    ...
    // 2. Dev. Anticipo
    // IMPORTANTE: Si estoy creando un Certificado de Anticipo, NO debo descontar anticipo.
    let ant = 0;
    if (tipo !== 'ANTICIPO') {
        let pctAnt = parseFloat(document.getElementById('inputAnticipoPct').value) || 0;
        ant = bruto * (pctAnt / 100);
    } else {
        // En anticipo, mostramos 0 descuento
        document.getElementById('inputAnticipoPct').value = 0;
    }
    ...
    // Auto-actualizar fuentes si ya están cargadas
    recalcularFuentes();
    validarFacturas();
}
