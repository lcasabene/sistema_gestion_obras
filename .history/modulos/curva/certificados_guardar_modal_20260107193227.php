...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // A. Inputs VISIBLES con máscara (Ej: "1.500,50")
        $f_moneda = function($v){ 
            if ($v === null || $v === '') return 0;
            return (float)str_replace(',','.', str_replace('.','',$v)); 
        };

        // B. Inputs OCULTOS o NUMÉRICOS (Ej: "1500.50" o "1500,50")
        ...
        // 2. Valores numéricos (NO confiamos en monto_neto del POST)
        ...
        // Si se sustituye fondo de reparo, por coherencia forzamos monto 0
        if ($fondo_reparo_sust) {
            $fondo_reparo_monto = 0;
        }

        // ✅ CÁLCULO ROBUSTO: Neto a pagar se calcula en servidor
        // (evita errores por parse/JS del navegador)
        $monto_neto = $monto_bruto - ($fondo_reparo_monto + $anticipo_desc + $multas);
        if ($monto_neto < 0) $monto_neto = 0;
