    ...
    while (($d = fgetcsv($handle, 8192, $delim)) !== FALSE) { 
        ...
        elseif ($tipo === 'tgf' || $tipo === 'anticipos' || $tipo === 'solicitado') {
            ...
            if (!$stmt) {
                ...
                $sql = "INSERT INTO sicopro_anticipos_tgf (ejer, fecha_vto, nro_tgf, actuacion, op_numero, proveedor, n_comprobante, importe, n_libramiento, contrasiento, quita, n_anticipo, f_anticipo, pesos, letras) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt = $pdo->prepare($sql);
            }
