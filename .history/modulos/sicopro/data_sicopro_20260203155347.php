...
// 5. BÚSQUEDA AVANZADA
    if (!empty($_POST['search']['value'])) {
        ...
        // Lógica para Expediente-Alcance (ej: 1234-5)
        if (strpos($searchVal, '-') !== false) {
            $partes = explode('-', $searchVal);
            $expediente = trim($partes[0]);
            $alcance = trim($partes[1]);

            $where .= " AND (movexpe LIKE :exp AND movalex LIKE :alc)";
            $params[':exp'] = "%$expediente%";
            $params[':alc'] = "%$alcance%";
        } else {
            // Búsqueda general normal
            $term = "%$searchVal%";
            $where .= " AND (
                movprov LIKE :s1 
                OR movrefe LIKE :s2 
            ...
            $params[':s1'] = $term; $params[':s2'] = $term;
            $params[':s3'] = $term; $params[':s4'] = $term;
            $params[':s5'] = $term; $params[':s6'] = $term;
            $params[':s7'] = $term; $params[':s8'] = $term;
        }
    }
