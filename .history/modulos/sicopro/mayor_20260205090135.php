 ...
 * Archivo: mayor.php
 * Versión Final: Letra grande, Tooltip Obra, Filtros Fecha, Link a Obra, MOVPROV visible, movfufi, movrefe y Fix SQL_BIG_SELECTS.
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $sql = "SELECT
                    ...
                    sub.MOVPROV, sub.movfufi, sub.movrefe,
                    IFNULL(e.razon_social, '---') as nombre_proveedor,
                    e.cuit as cuit_proveedor,
                    o.denominacion as nombre_obra,
                    SUM(sub.DEBE) as DEBE, SUM(sub.HABER) as HABER
                FROM (
                    SELECT movfeop, movtrti, movtrnu, MOVEXPE, MOVALEX, MOVPROV, movfufi, movrefe, MOVIMPO AS DEBE, 0 AS HABER
                    FROM sicopro_principal
                    WHERE 1=1 $f_debe
                    UNION ALL
                    SELECT movfeop, movtrti, movtrnu, MOVEXPE, MOVALEX, MOVPROV, movfufi, movrefe, 0 AS DEBE, MOVIMPO AS HABER
                    FROM sicopro_principal
                    WHERE 1=1 $f_haber
                ) AS sub
                LEFT JOIN empresas e ON sub.MOVPROV = e.codigo_proveedor
                LEFT JOIN obras o ON sub.MOVEXPE = o.expediente
                GROUP BY sub.movfeop, sub.movtrti, sub.movtrnu, sub.MOVEXPE, sub.MOVALEX, sub.MOVPROV, sub.movfufi, sub.movrefe
...
<head>
    ...
    <style>
        ...
        .table { font-size: 1rem; vertical-align: middle !important; }
        .table thead th { background-color: #f1f4f7; padding: 12px; font-size: 0.95rem; text-align: center; }
        ...
        .total-font { font-size: 1.2rem; font-weight: bold; }
    </style>
</head>
<body>

<div class="nav-top d-flex justify-content-between align-items-center mb-3 shadow">
    <h4 class="mb-0"><i class="fas fa-file-invoice-dollar me-2"></i>Mayor Contable</h4>
        ...
        <div class="card-body">
            <form method="POST" id="formFiltros" class="row g-3">
                ...
                <div class="col-md-2 d-flex align-items-end gap-2">
                    <button type="submit" class="btn btn-primary flex-grow-1"><i class="fas fa-search"></i> Consultar</button>
                    <a href="mayor.php" class="btn btn-secondary shadow-sm"><i class="fas fa-eraser"></i></a>
                </div>
            </form>
        </div>
    </div>
            ...
            <table id="tablaMayor" class="table table-striped table-hover w-100 border">
                ...
                <tbody>
                    <?php foreach ($resultados as $row): 
                        ...
                        $obra_info = !empty($row['nombre_obra']) ? "Obra: " . htmlspecialchars($row['nombre_obra']) : "Sin denominación en Obras";
                    ?>
                    <tr>
                        ...
                        <td class="text-center small"><?= htmlspecialchars($row['movfufi']) ?></td>
                        <td class="text-center small"><?= htmlspecialchars($row['movrefe']) ?></td>
                        <td>
                            <div class="exp-container">
                                ...
                                <a href="../obras/obras_listado.php?expediente=<?= urlencode($row['MOVEXPE']) ?>" 
                                   class="btn btn-outline-primary btn-sm px-1 py-0" title="Ver Obra">
                                    <i class="fas fa-eye" style="font-size: 0.8rem;"></i>
                                </a>
                            </div>
                        </td>
                        <td class="text-center fw-bold"><?= htmlspecialchars($row['MOVPROV']) ?></td>
                        <td>
                ...
                <tfoot class="table-dark total-font">
                    <tr>
                        <td colspan="8" class="text-end text-uppercase">Totales:</td>
