...
try {
    ...
    while (($data = fgetcsv($handle, 8192, $delimitador)) !== FALSE) { 
        ...
        elseif ($tipo_importacion === 'tgf' || $tipo_importacion === 'anticipos') {
            ...
            // Lógica especial para tu nuevo archivo
            $ejercicio = (int)$data[0]; // Limpia "2025,0" a 2025
            
            // Fecha VTO: detecta si es "02/01/jueves" o fecha normal
            $fechaVto = convertirFecha($data[1], 'dia_mes_texto', $ejercicio); 
            if (!$fechaVto) $fechaVto = convertirFecha($data[1], 'd/m/Y'); // Intento secundario

            // Fecha Anticipo: Columna 12 suele ser limpia "20/01/2025"
            $fechaAnt = convertirFecha($data[12], 'd/m/Y');

            $stmt->execute([
                $ejercicio, 
                $fechaVto, 
                $data[2], $data[3], $data[4], $data[5], $data[6],
                limpiarNumero($data[7], $formatoMoneda), // Usa formato detectado (argentino para ;)
