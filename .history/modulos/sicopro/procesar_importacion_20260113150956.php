...
try {
    ...
    while (($d = fgetcsv($handle, 8192, $delim)) !== FALSE) { 
        ...
        elseif ($tipo_seleccionado === 'liquidaciones') {
            ...
            if (!$stmt) {
                $sql = "INSERT INTO sicopro_liquidaciones (fecha, expediente, gedo, op_sicopro, razon_social, nro_proveedor, imp_liq, fdo_rep, ret_suss, ret_gcias, ret_iibb, ret_otras, ret_multas, imp_a_pagar, observaciones, ref_expediente, tipo_documento, nro_docum) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt = $pdo->prepare($sql);
            }
            $stmt->execute([
