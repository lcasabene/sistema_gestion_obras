 ...
 * Archivo: mayor.php
 * Filtros por fecha, unión con tabla empresas y botón limpiar.
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($desde)) {
        ...
        $params[':desde_1'] = $desde;
        $params[':desde_2'] = $desde;
    }
    if (!empty($hasta)) {
        $f_debe .= " AND MOVFEOP <= :hasta_1";
        $f_haber .= " AND MOVFEOP <= :hasta_2";
        $params[':hasta_1'] = $hasta;
        $params[':hasta_2'] = $hasta;
    }

    $sql = "SELECT 
                sub.movfeop, sub.MOVEXPE, sub.MOVALEX, 
...
<div class="nav-top d-flex justify-content-between align-items-center mb-3 shadow-sm">
    <h5 class="mb-0"><i class="fas fa-file-invoice-dollar me-2"></i>Mayor Contable</h5>
    ...
    <div class="card shadow-sm mb-3">
        <div class="card-body py-2">
            <form method="POST" id="formFiltros" class="row g-2">
                ...
                <div class="col-md-2">
                    <label class="small">Desde (MOVFEOP)</label>
                    <input type="date" name="f_desde" class="form-control form-control-sm" value="<?= htmlspecialchars($_POST['f_desde'] ?? '') ?>">
                </div>
                <div class="col-md-2">
                    <label class="small">Hasta (MOVFEOP)</label>
                    <input type="date" name="f_hasta" class="form-control form-control-sm" value="<?= htmlspecialchars($_POST['f_hasta'] ?? '') ?>">
                </div>
                <div class="col-md-3 d-flex align-items-end gap-2">
                    <button type="submit" class="btn btn-primary btn-sm flex-grow-1"><i class="fas fa-search"></i> Consultar</button>
                    <a href="mayor.php" class="btn btn-secondary btn-sm"><i class="fas fa-eraser"></i> Limpiar</a>
                </div>
            </form>
        </div>
    </div>
        ...
        <div class="card-body">
            <table id="tablaMayor" class="table table-striped table-hover table-sm w-100">
                ...
                <tfoot class="table-light fw-bold">
                    <tr>
                        <td colspan="4" class="text-end uppercase">Totales Finales:</td>
...
$(document).ready(function() {
    $('#tablaMayor').DataTable({
        ...
        buttons: [{
            ...
            footer: true,
            title: 'Mayor_Contable_<?= date("Ymd") ?>'
        }],
