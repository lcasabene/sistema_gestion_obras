...
try {
    ...
    // 2. LOGICA DE HEADER Y PRE-ESCANEO (Variable según el tipo)
    $anios_en_archivo = [];
    $posicion_inicio_datos = 0;
    
    // SKIP HEADERS: Depende del archivo
    if ($tipo === 'LIQUIDACIONES') {
        // Liquidaciones tiene basura arriba. Buscamos la línea que empieza con "N°"
        while (($row = fgetcsv($handle, 0, $delimiter)) !== FALSE) {
            // Convertir a string para buscar
            $linea = implode(';', $row); 
            // Si la primera columna es "N°" o similar, paramos.
            if (stripos($row[0], 'N°') !== false || stripos($row[0], 'N?') !== false) {
                break; // El puntero queda listo para leer datos en la siguiente iteración
            }
        }
    } elseif ($tipo === 'SIGUE' || $tipo === 'SICOPRO' || $tipo === 'TOTAL_ANTICIPADO' || $tipo === 'SOLICITADO' || $tipo === 'SIN_PAGO') {
    ...
    if (empty($anios_a_borrar)) throw new Exception("No se pudo detectar el año en el archivo.");
    ...
    while (($data = fgetcsv($handle, 0, $delimiter)) !== FALSE) {
        ...
        if ($tipo === 'SICOPRO') {
            ...
            // Mapeo LIQUIDACIONES (19 cols en CSV)
            // N°,FECHA,EXPEDIENTE,GEDO,OP,RAZON,PROV,IMP,FDO,SUSS,GCIAS,IIBB,OTRAS,MULTAS,PAGAR,OBS,REF,TIPO,DOC
            // SQL: ejer, nro, fecha, ...
            $data = array_pad($data, 19, null); // Asegurar longitud
            ...
            // Mapeo SIGUE (11 cols en CSV)
            // LIQN;Numero;Fecha;Nro.Pago;Tipo;Debito;Credito;Moneda;Importe;ObsLote;ObsTransf
            $data = array_pad($data, 11, null);

            $fecha = limpiar_fecha($data[2]); // Col 2 es Fecha
            $anio = $fecha ? date('Y', strtotime($fecha)) : 0;
    ...
    header('Location: importar.php?status=error&msg=' . urlencode("Error: " . substr($e->getMessage(), 0, 150)));
}