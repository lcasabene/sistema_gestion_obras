...
try {
 ...
 // ---------------------------------------------------------
        ...
        elseif ($tipo === 'tgf' || $tipo === 'anticipos' || $tipo === 'solicitado') {
            ...
            if (!$stmt) {
                // AHORA SÍ: Incluye n_libramiento (Total 15 campos)
            ...
            // Array con los 15 datos (incluyendo $d[8])
            $stmt->execute([
                $ejercicio,                         // 1. ejer
                $fechaVto,                          // 2. vto_comp
                $d[2],                              // 3. nro_tgf
                ...
                $d[5],                              // 6. proveedor
                $d[6],                              // 7. n_comp
                limpiarNumero($d[7], $fmtMoneda),   // 8. importe
                $d[8],                              // 9. n_libramiento (Dato del CSV)
                $d[9],                              // 10. contrasiento
                $d[10],                             // 11. quita
                $d[11],                             // 12. n_anticipo
                $fechaAnt,                          // 13. f_anticipo
                $d[13],                             // 14. pesos
                $d[14] ?? ''                        // 15. letras
            ]);
            $filas++;
