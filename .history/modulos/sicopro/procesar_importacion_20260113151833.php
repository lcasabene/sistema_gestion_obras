...
try {
    ...
    while (($d = fgetcsv($handle, 8192, $delim)) !== FALSE) { 
        ...
        if ($tipo_seleccionado === 'sigue') {
            ...
            if (!$stmt) {
                $sql = "INSERT INTO sicopro_sigue (numero_transaccion, fecha_pago, nro_pago_sistema, tipo, debito, credito, moneda, importe, beneficiario, observacion_lote, observacion_transferencia) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        elseif ($tipo_seleccionado === 'liquidaciones') {
            ...
            if (!$stmt) {
                // SE AGREGÓ nro_liquidacion AL INICIO (Total 19 campos)
                $sql = "INSERT INTO sicopro_liquidaciones (
                            nro_liquidacion, fecha, expediente, gedo, op_sicopro, razon_social, 
            ...
            // SE AGREGÓ $d[0] AL ARRAY
            $stmt->execute([
                $d[0],                                  // 1. nro_liquidacion (Columna 0 del CSV)
                convertirFecha($d[1], 'd/m/Y'),         // 2. fecha
                $d[2],                                  // 3. expediente
                $d[3],                                  // 4. gedo
                $d[4],                                  // 5. op_sicopro
                $d[5],                                  // 6. razon_social
                $d[6],                                  // 7. nro_proveedor
                limpiarNumero($d[7], 'argentino'),      // 8. imp_liq
                limpiarNumero($d[8], 'argentino'),      // 9. fdo_rep
                limpiarNumero($d[9], 'argentino'),      // 10. ret_suss
                limpiarNumero($d[10], 'argentino'),     // 11. ret_gcias
                limpiarNumero($d[11], 'argentino'),     // 12. ret_iibb
                limpiarNumero($d[12], 'argentino'),     // 13. ret_otras
                limpiarNumero($d[13], 'argentino'),     // 14. ret_multas
                limpiarNumero($d[14], 'argentino'),     // 15. imp_a_pagar
                utf8_encode($d[15] ?? ''),              // 16. observaciones
                $d[16] ?? '',                           // 17. ref_expediente
                $d[17] ?? '',                           // 18. tipo_documento
                $d[18] ?? ''                            // 19. nro_docum
            ]);
            $filas++;
        }
