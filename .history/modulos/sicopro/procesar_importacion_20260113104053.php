    ...
    while (($d = fgetcsv($handle, 8192, $delim)) !== FALSE) { 
        ...
        elseif ($tipo === 'tgf' || $tipo === 'anticipos' || $tipo === 'solicitado') {
            ...
            if (!$stmt) {
                ...
                $sql = "INSERT INTO sicopro_anticipos_tgf (ejer, vto_comp, nro_tgf, actuacion, o_pago, proveedor,
                 n_comp, importe,  contrasiento, quita, n_anticipo, f_anticipo, pesos, letras)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt = $pdo->prepare($sql);
            }
