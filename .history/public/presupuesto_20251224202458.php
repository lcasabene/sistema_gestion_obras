...
// 2. Determinar Versión Actual y Versión de Referencia (Comparativa)
$version_actual = $_GET['version'] ?? ($versiones[0] ?? null);
$version_ref = $_GET['version_ref'] ?? null; // Nueva versión para comparar
...
if ($version_actual) {
    ...
    $registros = $stmt->fetchAll(PDO::FETCH_UNIQUE | PDO::FETCH_ASSOC); 
    // Usamos FETCH_UNIQUE asumiendo que la combinación de clasificadores identifica la fila
}

// 4. Consultar registros de la versión de referencia (si existe)
$registros_ref = [];
if ($version_ref) {
    $stmtRef = $pdo->prepare("SELECT * FROM presupuesto_ejecucion WHERE fecha_carga = ? ORDER BY id ASC");
    $stmtRef->execute([$version_ref]);
    $registros_ref = $stmtRef->fetchAll(PDO::FETCH_UNIQUE | PDO::FETCH_ASSOC);
}
...
<style>
    ...
    .diff-pos { color: #dc3545; font-size: 0.75rem; font-weight: bold; } /* Aumento de gasto */
    .diff-neg { color: #198754; font-size: 0.75rem; font-weight: bold; } /* Ahorro/Baja */
</style>
    ...
    <div class="card shadow-sm mb-4 border-0">
        <div class="card-body bg-light border-bottom">
            <form id="formFiltros" method="GET" class="row g-3 align-items-end">
                <div class="col-md-3">
                    <label class="form-label small fw-bold">Versión a Analizar:</label>
                ...
                <div class="col-md-3">
                    <label class="form-label small fw-bold text-primary">Comparar contra:</label>
                ...
                <div class="col-md-6 text-end">
                    <div class="btn-group shadow-sm">
                        <button type="button" id="btnFiltroCritico" class="btn btn-outline-danger btn-sm">Ver Críticos</button>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table id="tablaPresupuesto" class="table table-hover table-sm mb-0">
                    <thead class="table-dark">
                        <tr>
                            ...
                            <th class="text-end">Diferencia</th> <th class="text-center">%</th>
                            <th class="text-center no-export">Acción</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($registros as $id => $r): 
                            ...
                            $diff_html = "";
                            if ($version_ref && isset($registros_ref[$id])) {
                                $ejec_ref = (float)$registros_ref[$id]['monto_ejec'];
                                $diff = $ejec - $ejec_ref;
                        ...
                        <tr class="<?= ($pct >= 80) ? 'table-danger' : '' ?>">
                            ...
                            <td class="text-end <?= ($pct >= 80) ? 'text-danger fw-bold' : '' ?>">
                            ...
                            <td class="text-center">
                                <span class="badge <?= ($pct >= 80) ? 'bg-danger' : 'bg-info text-dark' ?>">
                                    <?= number_format($pct, 0) ?>%
                                </span>
                            </td>
...
$(document).ready(function() {
    ...
    $('#btnFiltroCritico').on('click', function() {
        if ($(this).hasClass('active')) {
            $(this).removeClass('active').addClass('btn-outline-danger').text('Ver Críticos');
            table.column(6).search('').draw();
        } else {
            $(this).addClass('active').addClass('btn-danger').text('Ver Todos');
            table.column(6).search('%').draw(); // Asumiendo que las alertas tienen el símbolo %
            // O filtrar específicamente por clase si se desea más precisión
        }
    });
    ...
    $('.btn-detalle').on('click', function() {
        ...
        for (const [key, value] of Object.entries(datos)) {
            if(key === 'id') continue;
            html += `<div class="col-md-3"><div class="card h-100 border-0 shadow-sm"><div class="card-body p-2"><label class="text-muted fw-bold small text-uppercase">${key.replace('_',' ')}</label><div class="small text-dark text-break">${value || '-'}</div></div></div></div>`;
        }
