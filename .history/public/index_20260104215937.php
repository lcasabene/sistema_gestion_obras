...
// --- 1. CONFIGURACIÓN DEL FILTRO DE AÑO ---
$anio_actual = date('Y');
$anio_filtro = isset($_GET['anio']) ? (int)$_GET['anio'] : $anio_actual;

// Obtener lista de años disponibles (buscando en presupuesto_ejecucion)
$anios_disponibles = [];
try {
    // Buscamos los años distintos en la tabla de ejecución
    ...
    // Si no devuelve nada, al menos mostramos el año actual
...
$kpi = [
  'obras_activas' => 0,
  'fecha_corte' => 'Sin datos', // Texto para mostrar la fecha de los datos
...
try {
    // A. Obras Activas (Independiente del presupuesto, cuenta obras en ejecución hoy)
    ...
    // B. BUSCAR LA ÚLTIMA FECHA DE CARGA PARA EL AÑO SELECCIONADO
    // Ordenamos por ID DESC para obtener la carga más reciente real (evita errores si la fecha es texto)
    ...
    if ($ultima_fecha) {
        ...
        // C. SUMAR MONTOS USANDO ESA FECHA EXACTA
        $sql_presupuesto = "
            SELECT 
        ...
        $kpi['fecha_corte'] = "Sin cargas para $anio_filtro";
    }

} catch (Exception $e) {
    error_log("Error Dashboard: " . $e->getMessage());
    $kpi['fecha_corte'] = "Error de datos";
}

// Función auxiliar para calcular porcentajes sin dividir por cero
