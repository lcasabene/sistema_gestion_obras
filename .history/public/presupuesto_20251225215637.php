...
// =================================================================================
// 1. LOGICA DE FILTROS EN CASCADA (Ejercicio -> Versión -> Datos)
// =================================================================================

// A. Obtener Ejercicios disponibles (Años)
...
// Definir Ejercicio Actual (Por defecto el último disponible o el año actual)
$ejercicio_sel = $_GET['ejercicio'] ?? ($ejercicios[0] ?? date('Y'));

// B. Obtener Versiones disponibles SOLO del Ejercicio seleccionado
...
// C. Determinar Versión Actual y Referencia
// Si cambiamos de año y la versión vieja no existe en el nuevo año, seleccionamos la primera disponible
$version_actual = $_GET['version'] ?? ($versiones[0] ?? null);

// Validar que la versión seleccionada pertenezca al array de versiones (seguridad por si cambian el año manualmente)
...
// Consultar registros de la versión actual Y del ejercicio seleccionado
$registros = [];
if ($version_actual) {
    // Agregamos "AND ejer = ?" a la consulta
    ...
    foreach($res as $row) {
        // Clave única para comparación
        $row['clave_comparacion'] = $row['inci'].'-'.$row['ppal'].'-'.$row['ppar'].'-'.$row['fufi'].'-'.$row['denominacion1'];
        $registros[] = $row;
    }
}

// Consultar registros de referencia (Comparativa)
$registros_ref = [];
if ($version_ref) {
    // La referencia también debe respetar el ejercicio (salvo que quieras comparar años distintos, pero usualmente es interanual)
    $stmtRef = $pdo->prepare("SELECT * FROM presupuesto_ejecucion WHERE fecha_carga = ? AND ejer = ? ORDER BY id ASC");
    $stmtRef->execute([$version_ref, $ejercicio_sel]);
    ...
    <div class="card shadow-sm mb-4 border-0">
        <div class="card-body bg-light border-bottom">
            <form id="formFiltros" method="GET" class="row g-2 align-items-end">
                ...
                <div class="col-md-3">
                    <label class="form-label small fw-bold text-primary">Comparar con:</label>
