...
if ($version_actual) {
    ...
    foreach($res as $row) {
        // Generamos la clave para comparar, pero NO la usamos como índice del array principal
        // para evitar que se sobrescriban filas repetidas.
        $row['clave_comparacion'] = $row['inci'].'-'.$row['ppal'].'-'.$row['ppar'].'-'.$row['fufi'].'-'.$row['denominacion1'];
        
        // Guardamos indexando numéricamente (append) para tener las 195 filas
        $registros[] = $row;
    }
}

// 4. Consultar registros de la versión de referencia
// NOTA: Aquí sí mantenemos la clave como índice para poder buscar rápido la referencia.
// Advertencia: Si la versión de referencia tiene duplicados, solo tomará el último valor para comparar.
$registros_ref = [];
if ($version_ref) {
                ...
                <table id="tablaPresupuesto" class="table table-hover table-sm mb-0" style="width:100%">
                    ...
                    <tbody>
                        <?php foreach ($registros as $r): 
                            // Obtenemos la clave calculada anteriormente
...
$(document).ready(function() {
    ...
    $('.btn-detalle').on('click', function() {
        ...
        for (const [key, value] of Object.entries(datos)) {
            if(key === 'id' || key === 'clave_comparacion') continue; // Omitir clave interna
