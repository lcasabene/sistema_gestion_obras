...
// --- 1. CONFIGURACIÓN INTELIGENTE DE AÑO ---
$anio_sistema = date('Y');
$anios_disponibles = [];
$anio_por_defecto = $anio_sistema;

try {
    // Busca años disponibles
    ...
    // Si hay datos, el año por defecto es el MAYOR (ej: 2025). Si no, 2026.
...
// Filtro seleccionado (o por defecto)
...
$kpi = [
  ...
  'error_detalle' => '', // Variable para ver el error real si ocurre
  'presup_global_def' => 0, 'presup_global_ejec' => 0,
  'obras_gen_def' => 0,     'obras_gen_ejec' => 0,
  'obras_edu_def' => 0,     'obras_edu_ejec' => 0,
  'deuda_def' => 0,         'deuda_ejec' => 0
];
...
try {
    ...
    if ($ultima_fecha) {
        ...
        // C. SUMAR MONTOS (CORRECCIÓN: 'inci' en lugar de 'inc')
        $sql_presupuesto = "
            SELECT 
                ...
                -- Servicio de la Deuda (CORREGIDO: campo 'inci' = 7)
                SUM(CASE WHEN inci = 7 AND ppal in(1,2) THEN monto_def ELSE 0 END) as deuda_def,
                SUM(CASE WHEN inci = 7 AND ppal in(1,2) THEN monto_ejec ELSE 0 END) as deuda_ejec
    ...
    // Capturamos el error real para mostrarlo en pantalla si hace falta
    $kpi['fecha_corte'] = "Error SQL";
    $kpi['error_detalle'] = $e->getMessage(); 
}
...
<div class="container my-4">
    ...
    <?php if (!empty($kpi['error_detalle'])): ?>
        <div class="alert alert-danger">
            <strong>Debug Error:</strong> <?= htmlspecialchars($kpi['error_detalle']) ?>
        </div>
    <?php endif; ?>
                ...
                <div class="card-body">
                    ...
                    <div class="mt-2">
                        ...
                        <small class="text-muted" style="font-size: 0.70rem;">
                            Total: $ <?= number_format($kpi['presup_global_def']/1000000, 0, ',', '.') ?> M
                        </small>
                    </div>
                </div>
            </div>
        ...
        <div class="col-md-4">
            <div class="card h-100 shadow-sm hover-card">
                <div class="card-body text-center">
                    ...
                    <h5>Menu General</h5>
                    <p class="text-muted small">Acceso a Principales Módulos</p>
                    <a href="menu.php" class="btn btn-outline-primary w-100 fw-bold">Ir al Tablero</a>
                </div>
            </div>
        </div>
        <div class="col-md-4">
