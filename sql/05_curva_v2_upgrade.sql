...
-- ==========================================================
-- 1. TABLA: empresas (no existía en 01_schema.sql)
-- ==========================================================
CREATE TABLE IF NOT EXISTS empresas (
  ...
  INDEX idx_emp_cuit (cuit)
) ENGINE=InnoDB;

-- ==========================================================
-- 2. TABLA: fuentes_financiamiento - agregar columna 'codigo'
-- ==========================================================
-- El código usa ff.codigo en múltiples consultas
ALTER TABLE fuentes_financiamiento 
  ADD COLUMN IF NOT EXISTS codigo VARCHAR(20) DEFAULT NULL AFTER id;

-- Quitar UNIQUE de nombre si causa conflicto (codigo será el identificador)
-- ALTER TABLE fuentes_financiamiento DROP INDEX nombre;

-- ==========================================================
-- 3. TABLA: obras - agregar columnas faltantes
-- ==========================================================
ALTER TABLE obras
  ADD COLUMN IF NOT EXISTS empresa_id INT DEFAULT NULL AFTER estado_obra_id,
  ADD COLUMN IF NOT EXISTS anticipo_pct DECIMAL(6,3) DEFAULT 0 AFTER monto_actualizado,
  ADD COLUMN IF NOT EXISTS anticipo_monto DECIMAL(18,2) DEFAULT 0 AFTER anticipo_pct,
  ADD COLUMN IF NOT EXISTS periodo_base VARCHAR(7) DEFAULT NULL AFTER moneda,
  ADD COLUMN IF NOT EXISTS region VARCHAR(60) DEFAULT NULL AFTER ubicacion,
  ADD COLUMN IF NOT EXISTS organismo_requirente VARCHAR(200) DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS titularidad_terreno VARCHAR(200) DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS superficie_desarrollo VARCHAR(100) DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS caracteristicas_obra TEXT DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS memoria_objetivo TEXT DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS latitud VARCHAR(20) DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS longitud VARCHAR(20) DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS geojson_data LONGTEXT DEFAULT NULL;

-- ==========================================================
-- 4. TABLA: obra_fuentes_config (config de FUFI por obra)
-- ==========================================================
...
-- ==========================================================
-- 7. TABLA: curva_detalle - agregar columnas extendidas
--    (curva_generate.php inserta columnas adicionales)
-- ==========================================================
ALTER TABLE curva_detalle
  ADD COLUMN IF NOT EXISTS monto_bruto_plan DECIMAL(18,2) DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS anticipo_pago_plan DECIMAL(18,2) DEFAULT 0,
  ADD COLUMN IF NOT EXISTS anticipo_pago_modo VARCHAR(20) DEFAULT 'PARIPASSU',
  ADD COLUMN IF NOT EXISTS anticipo_pago_fuente_id INT DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS anticipo_recupero_plan DECIMAL(18,2) DEFAULT 0,
  ADD COLUMN IF NOT EXISTS anticipo_rec_modo VARCHAR(20) DEFAULT 'PARIPASSU',
  ADD COLUMN IF NOT EXISTS anticipo_rec_fuente_id INT DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS monto_neto_plan DECIMAL(18,2) DEFAULT NULL;
