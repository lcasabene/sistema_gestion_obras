<?php
// obras_form.php - Con Geolocalización y Traza GeoJSON (Modificado: Anticipo en General)
...
<style>
    #map { height: 500px; width: 100%; border-radius: 4px; border: 1px solid #ced4da; }
</style>

<div class="d-flex justify-content-between align-items-center mb-3">
  <div>
    <h3 class="mb-0 fw-bold text-primary"><?php echo $id>0 ? 'Editar Obra' : 'Nueva Obra'; ?></h3>
    <div class="text-muted small">Gestión técnica, presupuestaria, financiera y geográfica.</div>
  </div>
...
<form method="post" action="obras_guardar.php" class="card shadow-sm" id="formObra">
  <div class="card-body">
    ...
    <ul class="nav nav-tabs" role="tablist">
      ...
      <li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#tab_mapa" type="button" role="tab"><i class="bi bi-geo-alt-fill text-danger"></i> Mapa / Ubicación</button></li>
    ...
    <div class="tab-content pt-3">
          ...
          <div class="col-md-6">
            ...
            <select name="empresa_id" id="selectEmpresa" class="form-select" required>
              ...
              <?php foreach ($empresas as $emp): ?>
                <option value="<?php echo (int)$emp['id']; ?>" <?php echo ((int)$obra['empresa_id']==(int)$emp['id'])?'selected':''; ?>><?php echo htmlspecialchars($emp['cuit'] . ' - ' . $emp['razon_social']); ?></option>
              <?php endforeach; ?>
            </select>
          </div>
      ...
      <div class="tab-pane fade" id="tab_mapa" role="tabpanel">
        <div class="row g-3">
            <div class="col-md-12">
                <div class="alert alert-info py-2 small mb-2">
                    <i class="bi bi-info-circle-fill me-1"></i> Arrastre el marcador azul para definir la ubicación principal. Cargue un archivo GeoJSON para dibujar la traza (rutas/polígonos).
                    <br><strong>Colores de traza:</strong> El sistema pintará Verde si la propiedad <code>"estado": "ejecutado"</code>, Naranja si es <code>"en_ejecucion"</code> y Rojo si es <code>"pendiente"</code>.
                </div>
            </div>
            <div class="col-md-3">
                <label class="form-label small fw-bold">Latitud</label>
                <input type="text" name="latitud" id="latitud" class="form-control form-control-sm" readonly value="<?= htmlspecialchars($obra['latitud'] ?? '') ?>">
            </div>
            <div class="col-md-3">
                <label class="form-label small fw-bold">Longitud</label>
                <input type="text" name="longitud" id="longitud" class="form-control form-control-sm" readonly value="<?= htmlspecialchars($obra['longitud'] ?? '') ?>">
            </div>
            <div class="col-md-6">
                <label class="form-label small fw-bold">Cargar Traza (GeoJSON)</label>
                <div class="input-group input-group-sm">
                    <input type="file" id="fileGeoJSON" class="form-control" accept=".json,.geojson">
                    <button class="btn btn-outline-secondary" type="button" id="btnLoadJSON">Cargar al Mapa</button>
                    <?php if($obra['id'] > 0): ?>
                        <a href="obras_mapa.php?id=<?= $obra['id'] ?>" target="_blank" class="btn btn-primary"><i class="bi bi-arrows-fullscreen"></i> Ver Completo</a>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="col-md-12">
                <div id="map"></div>
                <textarea name="geojson_data" id="geojson_data" style="display:none;"><?= htmlspecialchars($obra['geojson_data'] ?? '') ?></textarea>
            </div>
        </div>
      </div>
...
$(document).ready(function() {
    ...
    // Coordenadas Iniciales (Neuquén por defecto o las guardadas)
    var initLat = parseFloat($('#latitud').val()) || -38.9516; 
    var initLng = parseFloat($('#longitud').val()) || -68.0591;
    var initZoom = ($('#latitud').val()) ? 13 : 10;
    ...
    // Marcador arrastrable (Ubicación de la obra)
    ...
    // Función para manejar el problema de renderizado del mapa en pestañas ocultas
    $('button[data-bs-toggle="tab"]').on('shown.bs.tab', function (e) {
        if (e.target.dataset.bsTarget === '#tab_mapa') {
            map.invalidateSize();
        }
    });

    // Estilos según estado
    function estiloTraza(feature) {
        var color = "#3388ff"; // Azul por defecto
        if (feature.properties && feature.properties.estado) {
            switch(feature.properties.estado) {
                case 'ejecutado': color = "green"; break;
                case 'en_ejecucion': color = "orange"; break;
    ...
    function cargarGeoJSONEnMapa(geojsonData) {
        if (geoLayer) map.removeLayer(geoLayer);
        try {
            var data = JSON.parse(geojsonData);
    ...
    // Cargar GeoJSON existente al abrir
    var existingJSON = $('#geojson_data').val();
    if (existingJSON && existingJSON.trim() !== '') {
        cargarGeoJSONEnMapa(existingJSON);
    }

    // Botón para cargar archivo local
    $('#btnLoadJSON').click(function() {
        var fileInput = document.getElementById('fileGeoJSON');
        ...
        if (file) {
            ...
            reader.onload = function (evt) {
                ...
                $('#geojson_data').val(jsonStr); // Guardar en textarea oculto
                cargarGeoJSONEnMapa(jsonStr);    // Mostrar en mapa
                alert('Traza cargada correctamente. Recuerde "Guardar Obra" para confirmar los cambios.');
            }
            reader.onerror = function (evt) {
                alert("Error al leer el archivo");
            }
        } else {
            alert("Seleccione un archivo .json o .geojson primero.");
        }
    });
});
</script>
