...
try {
    ...
    echo "<div class='container my-4'><div class='alert alert-danger'><strong>Error BD:</strong> " . $e->getMessage() . "</div></div>";
    $obras = [];
}
...
<style>
    ...
    .tooltip-inner { white-space: pre-wrap; text-align: left; max-width: 350px; }
</style>
    ...
    <div class="card mb-3 border-0 bg-light shadow-sm">
        <div class="card-body py-3">
            <form class="row g-3 align-items-end" method="get">
                <div class="col-auto d-flex align-items-center"><span class="fw-bold text-secondary"><i class="bi bi-funnel"></i> Filtros:</span></div>
                <div class="col-md-4">
                    ...
                    <select name="tipo[]" class="form-select select2-multiple" multiple>
                        <?php foreach($tipos as $t): $selected = in_array($t['id'], $tipos_sel) ? 'selected' : ''; ?>
                ...
                <div class="col-md-4">
                    ...
                    <select name="estado[]" class="form-select select2-multiple" multiple>
                        <?php foreach($estados as $est): $selected = in_array($est['id'], $estados_sel) ? 'selected' : ''; ?>
                            <option value="<?= $est['id'] ?>" <?= $selected ?>><?= $est['nombre'] ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-auto">
                    <button type="submit" class="btn btn-primary fw-bold"><i class="bi bi-search"></i> Aplicar</button>
                    <?php if(!empty($tipos_sel) || !empty($estados_sel)): ?>
                        <a href="obras_listado.php" class="btn btn-outline-secondary" title="Limpiar"><i class="bi bi-x-lg"></i></a>
                    <?php endif; ?>
                </div>
            </form>
        </div>
                ...
                <table id="tablaObras" class="table table-hover align-middle mb-0 table-vcenter" style="font-size: 0.95rem; width:100%">
                    ...
                    <tbody>
                        <?php foreach($obras as $o): 
                            ...
                            } else { $colorDesvio = 'secondary'; $txtDesvio = 'S/ Curva'; }
                            ...
                            $tooltipDetalle = htmlspecialchars($o['tooltip_avance'] ?? '');
                            if(empty($tooltipDetalle)) $tooltipDetalle = "Sin avance físico (o solo anticipos)";
                        ?>
                        <tr>
                            <td>
                                ...
                                <div class="d-flex gap-2">
                                    ...
                                    <span class="badge bg-info bg-opacity-10 text-info border border-info text-xs"><i class="bi bi-calendar-event"></i> Fin: <?= $fechaFin ?></span>
                                </div>
                            </td>
                            ...
                            <td>
                                <div class="d-flex justify-content-between small mb-1 fw-bold">
                                    <span class="text-success cursor-pointer" data-bs-toggle="tooltip" data-bs-html="true" title="<b>Desglose:</b><br><?= $tooltipDetalle ?>" style="cursor:help; text-decoration: underline dotted;">R: <?= number_format($real,2) ?>%</span>
                                ...
                                <div class="progress shadow-sm mb-1" style="height: 6px; background-color: #e9ecef;">
                                    ...
                                    <div class="position-absolute border-start border-dark border-2" style="left: <?= min($teorico, 100) ?>%; height: 6px; opacity: 0.6;"></div>
                                </div>
                                <div class="d-flex justify-content-between align-items-center">
                                    <small class="text-<?= $colorDesvio ?> fw-bold text-xs"><?= $txtDesvio ?> (<?= ($desvio>0?'+':'').number_format($desvio,1) ?>%)</small>
                                    <small class="text-muted text-xs bg-light border px-1 rounded" title="Último Periodo"><i class="bi bi-clock-history"></i> <?= $ultimoPer ?></small>
                                </div>
                            </td>
                            <td class="text-end small">
                                <div class="text-muted text-xs mb-1">Orig: $ <?= number_format($o['monto_original'], 0, ',', '.') ?></div>
                                <div class="fw-bold text-dark mb-1">Act: $ <?= number_format($o['monto_actualizado'], 0, ',', '.') ?></div>
                                <div class="text-success text-xs bg-success bg-opacity-10 px-1 rounded d-inline-block">Pagado: $ <?= number_format($o['total_certificado'], 0, ',', '.') ?></div>
                            </td>
                            <td class="text-center">
                                <div class="btn-group" role="group">
                                    <a href="obras_mapa.php?id=<?= $o['id'] ?>" class="btn btn-outline-danger btn-action shadow-sm" title="Ver Mapa" data-bs-toggle="tooltip" target="_blank"><i class="bi bi-geo-alt-fill fs-6"></i></a>
                                    <a href="../certificados/certificados_form.php?obra_id=<?= $o['id'] ?>" class="btn btn-success btn-action shadow-sm text-white" title="Nuevo Certificado" data-bs-toggle="tooltip"><i class="bi bi-plus-lg fs-6"></i></a>
                                    <?php if($o['version_id']): ?>
                                        <a href="../curva/curva_ver.php?version_id=<?= $o['version_id'] ?>" class="btn btn-primary btn-action shadow-sm" title="Ver Curva" data-bs-toggle="tooltip"><i class="bi bi-graph-up fs-6"></i></a>
                                    <?php else: ?>
                                        <a href="../curva/curva_form_generate.php?obra_id=<?= $o['id'] ?>" class="btn btn-warning btn-action shadow-sm text-dark" title="Generar Curva" data-bs-toggle="tooltip"><i class="bi bi-magic fs-6"></i></a>
                                    <?php endif; ?>
                                    <a href="obras_evento.php?obra_id=<?= $o['id'] ?>" class="btn btn-info btn-action shadow-sm text-white" title="Eventos" data-bs-toggle="tooltip"><i class="bi bi-calendar-range fs-6"></i></a>
                                    <a href="obras_form.php?id=<?= $o['id'] ?>" class="btn btn-dark btn-action shadow-sm" title="Editar" data-bs-toggle="tooltip"><i class="bi bi-pencil-fill fs-6"></i></a>
                                    <a href="obras_ficha_pdf.php?id=<?= $o['id'] ?>" class="btn btn-secondary btn-action shadow-sm" title="Generar Ficha PDF" data-bs-toggle="tooltip" target="_blank">
...
$(document).ready(function() {
    $('.select2-multiple').select2({ theme: 'bootstrap-5', width: '100%', placeholder: 'Seleccione...', closeOnSelect: false, allowClear: true });
    $('#tablaObras').DataTable({
        language: { url: '//cdn.datatables.net/plug-ins/1.13.4/i18n/es-ES.json' },
        order: [[0, 'desc']], columnDefs: [{ orderable: false, targets: 4 }],
        pageLength: 10, lengthMenu: [10, 25, 50, 100], stateSave: true 
    });
