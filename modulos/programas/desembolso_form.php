...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$programa_id) {
        ...
        if ($id) {
            $pdo->prepare("UPDATE programa_desembolsos SET obra_id=?,fecha=?,importe=?,moneda=?,observaciones=? WHERE id=?")
                ->execute([$obra_id, $data['fecha'], $data['importe'], $data['moneda'], $data['observaciones'], $id]);
        } else {
            $pdo->prepare("INSERT INTO programa_desembolsos (programa_id,obra_id,fecha,importe,moneda,observaciones,usuario_id) VALUES (?,?,?,?,?,?,?)")
                ->execute([$programa_id, $obra_id, $data['fecha'], $data['importe'], $data['moneda'], $data['observaciones'], $data['usuario_id']]);
        }
