...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $f_moneda = function($v){
            ...
            $s = trim((string)$v);
            // "1.600.000.000,00" -> "1600000000.00"
            $s = str_replace('.', '', $s);
            $s = str_replace(',', '.', $s);
            return (float)$s;
        };
        ...
        // 1) Montos visibles (vienen con separadores)
        ...
        if ($cert_id == 0) {
            ...
            $stmt->execute([
                ...
                ':multas'   => $multas,
                ':neto'     => $monto_neto,     // ✅ neto recalculado
